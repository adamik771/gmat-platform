...
- **1:** corrected the verified ticket/backsolving, signed-number, DI distractor/units, mixture and pacing examples; replaced off-topic Statistics/Probability graded-set pins. Keys and bank IDs remain stable; the erroneous mixture example's choice C is now the actual answer, 10/3 liters. Removed the unvalidated target-score-to-practice-accuracy requirement while preserving score goals and actual results. Targets inside graded sets are labelled study guidelines, not official score conversions.
- **2:** DS delivery uses a seeded shuffle without answer-key rotation, small-set answer quotas or hard streak caps. Difficulty, exact freshness ordering, non-DS slots and shared contexts stay intact. Mock selection no longer excludes a DS answer solely because that letter already appeared. The curated standalone DS pool-selection policy is unchanged (its membership can change after relabelling); this does not claim the bank has a perfectly equal answer distribution.
- **3:** item QA counts one eligible first recorded response per student/question, excluding known hints/review/redo before calculating sample statistics. It uses Wilson intervals and an explicitly labelled difference between groups' performance on OTHER same-section items, not a point-biserial coefficient. Small samples do not declare items broken or the bank healthy. The service-role aggregate loader checks trusted admin access before cross-student reads; normal student RLS is untouched. Pagination is bounded, date-plus-ID ordered, microsecond-aware and fail-closed. No student identities are returned by the aggregate loader.
- **4:** sampling routes combine own-user practice history with recorded chapter interactions, including versioned graded-set IDs. Reading alone is not question exposure. Legacy identified interactions without timestamps are treated as seen at an unknown old date, never as newly answered today; legacy aggregate counts without IDs cannot be reconstructed. Two-device merge tests retain exposure, notes, reading and set identities. Failed history reads show a warning rather than claiming complete freshness. No new growing state is placed in auth metadata.
- **5:** selected sidebar/instruction text contrast, 14px sidebar labels and keyboard focus; Course/Chapters copy distinguishes the main interactive path from optional guides; empty practice entries link to the chapter instead of promising a coming test. No prep redesign, route deletion, chapter chunking or content removal.
- **7:** 11 targeted Advanced questions reviewed; ten relabelled Intermediate and algebra-q137 retained Advanced. No prompts, explanations or keys changed as part of this relabel sub-batch. All 2,007 bank questions remain. Item-by-item reasoning is in `GMAT_CONTENT_AUDIT.md`; human editorial approval is pending in `CONTENT_PROVENANCE.md`.

**Compatibility:** changed in-chapter sets carry old IDs. Existing unfinished sets can finish against their original questions, with old results shown as Previous set rather than credited to replacement questions; new checkpoints/results persist their IDs. The reader freezes its active deck. Comparing the actual allocator against HEAD's pre-batch content identified 21 changed standalone test memberships. `previous-practice-tests.ts` preserves those exact memberships for local/server snapshot recovery only; new attempts use current decks. Existing user/route/TTL/answer-index/hint validation and the entitlement gate remain in force. Restored old tests explicitly announce their previous version. This is compatibility for this batch, not permission to load arbitrary old decks. Keep the registry until these snapshots cannot need recovery. Saved completed session history is not deleted.

**Verification so far:** browser fixtures used real components with synthetic local-only data, not a signed-in production account. Desktop 1440px and mobile 390px practice/course/chapter/builder views checked; no document overflow on mobile. Old graded set resumed Q2, graded correctly, retained a note and old question IDs, and displayed Previous set; a new corrected set resumed Q2 after reload. History-failure builder stayed usable without false repeat estimates. Temporary fixture removed before final gate. The fixture itself produced a React child-key warning; it was not part of the shipped route tree. Screenshots inspected, not saved as a screenshot archive. Cross-device behavior is tested at merge/loader level, not with two real signed-in browsers. Live Supabase query/schema behavior and production deployment remain unverified in this checkout.

**Standalone recovery browser follow-up:** a second temporary fixture rendered real SessionClient with synthetic data and intercepted all local API writes. Both a local snapshot and a server-prop snapshot resumed the old test at Q2, with the previous-version message. Visiting Q1 retained its submitted state and 52-second answering time; a fresh attempt started at Q1 with unsubmitted answers. Mobile 390px banner checked without overflow. No new console error during this follow-up. Fixture removed afterward.

**Final local gate:** `npm run check -- --maxWorkers=2` passes: content 0 errors/0 warnings/0 info, TypeScript clean, 808/808 tests across 103 files. The first unrestricted run hit the existing state-size stress test's 5-second timeout under load; no timeout, assertion or stress-test workload was relaxed. Limiting worker concurrency passed the complete suite. `npm run lint` has zero errors and only the pre-existing `scripts/send-consult-batch.ts:115` warning. `npm run build` passes with 174 static pages and no fixture route. `git diff --check` is clean. Build-time live Supabase schema validation was skipped because this checkout has no database environment. No new environment variable is introduced; the admin report needs the existing server-only Supabase service configuration.
...
- **F01-F04:** sample layout clears the 73px public header; saved chapter-practice actions use the actual saved session ID for results/answers while Resume and Retake remain separate; shared timezone-aware final-week review recommendation is used by Today, Plan, and Exams (including exam day and no-baseline cases); recursive font token fixed, with Inter for interface text and Fraunces limited to selected public headings.
- **F05-F08:** six primary destinations with retained secondary routes; task-first Today and compact remaining setup step; explicit reading/practice metric scopes; primary interactive curriculum plus supporting readings/references; practice filters and available recommendation, with section/subskill-aware matching; a wider reader without the permanent right rail, one reading-progress definition, contents drawer below 1200px, retained focus/light/dark modes. Shared content inventory: 62 interactive chapters, 29 supporting readings, 22 references, 2,007 bank questions; chapter-test inventory remains separately scoped.
- **F09-F12:** truthful early-release/trial/reservation copy and aligned plan cards; no new checkout flow; actual review reasons rather than calling every due item a mistake; available analytics evidence first with sample/window/source labels; shorter homepage with an actual authored sample question, consolidated navigation, one lead form, founder-only evidence, and clearer public resources/contact pages. Prices and consent rules unchanged. All original chapter/question files untouched.
- Settings adds an exam-date change preview and explicit confirmation using the existing profile write. Weekly hours are displayed truthfully but remain read-only here; no profile API contract was expanded. The preview describes input changes, not an invented regenerated task plan.

**Verification:** `npm run check` passes: content validation 0 errors/0 warnings/0 info, TypeScript clean, 836 tests across 102 files. `npm run lint` has 0 errors and the one pre-existing warning in `scripts/send-consult-batch.ts:115`; `git diff --check` is clean. `npm run build` passes with locked Next 16.3.1/Turbopack and 174 static pages. Live Supabase schema validation was skipped by the build because this isolated worktree has no database environment. Cross-reviews checked preserved reader/session state and routing; a wrong-section recommendation and a fully-answered-but-unfinished Resume edge were corrected and regression-tested.

**Browser evidence:** local preview `http://127.0.0.1:3431/`; homepage, pricing, resources, and the three canonical sample routes checked at 1440/1280/1024/768/390/320px. Pricing's offscreen screen-reader labels and a Quant sample table overflow were fixed; final narrow checks report document width equal to viewport width. Header clearance and one H1 verified for all samples. Public/app drawers close with Escape and restore trigger focus; reader contents drawer and persisted light/dark preference checked; correct/incorrect sample answers and reset checked. Today (new/nearly-complete/active/final-week), practice filter/no-results/saved-results destinations, reader, and analytics used explicitly labelled local demonstration fixtures rendering real components. That temporary fixture route was deleted before the final build. No student attempts or form submissions were made. Screenshots were inspected during QA, not saved as a before/after image archive. Homepage console check returned no errors.

**Not a complete release clearance:** authenticated saves, expired access, real-account login/recovery and settings confirmation still need a disposable signed-in preview account; Safari/Firefox, real-phone and screen-reader coverage, comparable before/after performance measurements and field Web Vitals remain unverified. No speed or conversion uplift is claimed. Selected-plan reservations require a proper `lead_captures.selected_plan_id` migration plus API/UI validation; current copy deliberately promises a general founding-rate reservation and later plan choice. A full rescheduled-task preview and weekly-hours editor are not implemented. Runner pre-start configuration, clearer adaptive-level versus question-difficulty labels, and preventing Exam-to-Study reveal mid-run remain a separate behavior-tested follow-up; this redo does not alter runner timing, grading, or persistence.

This file exists so a fresh Claude chat can pick up exactly where the previous one left off. Read it first, then continue.

## CONTEXT SWITCH — 2026-08-21 (Advanced question calibration; branch `content/advanced-question-calibration-20260821`)

High-confidence Quant/DI Advanced-tier calibration completed on production base `66aae60`. Applied the existing rubric (multiple dependent constraints / non-obvious inference / meaningful method switch; never difficulty-by-verbosity): **37 obvious one-tier inflations changed Hard→Medium**, three genuinely multi-constraint Hard questions added (`arithmetic-q125/q126`, `backsolving-q53`), and five affected hard chapter sets received in-place stronger replacements so lengths/positions remain stable for active users. Advanced pool is 616→582 after the three additions; structural direct-method flags are 63→29, with the remaining candidates mostly passage-dependent Verbal items that were not mechanically relabelled. `graphics-interpretation-q47` now clarifies that answer option E represents Product F. Full rationale and item categories are appended to `GMAT_CONTENT_AUDIT.md`; regression coverage is in `tests/question-difficulty-calibration.test.ts`.

Exact-tree gate: content validator 0 errors (106 existing warnings / 72 info), TypeScript clean, **60 files / 583 tests pass**, eslint 0 errors (one pre-existing warning in `scripts/send-consult-batch.ts`), and `next build --webpack` succeeds with all 170 static pages. Default Turbopack build cannot follow the isolated worktree's cross-root `node_modules` symlink; webpack build is the valid production artifact check. No migration or environment change is required.

## CONTEXT SWITCH — 2026-08-20 (IP/copyright/trademark hardening; branch `legal/ip-compliance-hardening-20260820`; UNCOMMITTED pending Adam review)

Repository-side IP risk minimization completed on current production base `06391d4`. Public Terms ownership language now excludes identified third-party materials/trademarks and covers owned-or-licensed original selection, arrangement, design, and software; the site-wide, auth, referral, founding, acquisition, error-log, and transactional-email GMAC notices now use the accurate four-mark wording plus a clear independent/no-endorsement statement. Unsupported named-competitor score comparisons were removed from the diagnostic/mock lessons and score calculator. `/resources` now attributes `forall x: Calgary` to Aaron Thomas-Bolduc and Richard Zach under CC BY 4.0, and labels the Athabasca book CC BY-NC-SA 4.0 as an external, noncommercial reference not incorporated into the paid course.

New operational controls: `CONTENT_PROVENANCE.md` is an honest batch-level evidence ledger (historical limitations, OER/license register, source-isolation rule, future-batch template, human-review requirements, takedown procedure, quarterly audit checklist/log, and owner-only legal actions); matching permanent rules were added to `AGENTS.md` and `QUESTION_TAXONOMY.md`. `scripts/generate-third-party-notices.mjs` walks the installed production dependency graph deterministically; `public/third-party-notices.txt` currently records 358 packages and is linked from Terms/footer; `npm run notices:check` prevents stale notices. `tests/ip-compliance.test.ts` locks the trademark notice, claim removals, OER labels, provenance controls, and public notice artifact.

Verification on the exact uncommitted tree: content validator 0 errors (109 existing warnings / 72 info), tsc clean, 56 test files / 561 tests pass, full eslint 0 errors (one pre-existing warning in untouched `scripts/send-consult-batch.ts`), notices current, `next build --webpack` successful (Turbopack itself is blocked in this execution sandbox by its internal worker-port mechanism; first attempt with a cross-worktree node_modules symlink was also rejected by Next 16.2.3, so a clean local `npm ci` was used), all 170 static routes generated. Local production smoke: `/terms`, `/resources`, `/third-party-notices.txt` all 200 and corrected strings present. No commit/push yet because repository rules require explicit approval.

External/owner actions remain intentionally unclaimed: ask Norwegian IP counsel to review the brand/domain/Terms/ownership chain; commission trademark clearance searches in intended markets and consider registering the distinctive ZAKARIAN word/logo elements; decide personal-vs-company IP ownership and execute an assignment if needed; get written permission before commercial adaptation of any noncommercial resource; preserve AI-service terms, receipts/licenses, design sources, and dated human editorial records. The repository audit found no clear copied item, official/competitor screenshot, or hotlinked product image, but it is risk reduction rather than legal clearance.

## CONTEXT SWITCH — 2026-07-21 (product-excellence batch: 6-panel audit + ~90 fixes across review loop / durability / plan-metrics / a11y-mobile; branch `feat/product-excellence-batch`, pushed — Adam merges)

Six investigation panels (30 read-only agents) + 5 adversarial reviewers over base `a02aa04`, then 4 narrow commits + a 5-panel post-implementation review + lead browser QA. **Read `PRODUCT_EXCELLENCE_AUDIT.md` (verified findings + roadmap) and `PRODUCT_EXCELLENCE_REPORT.md` (what shipped, evidence, risks) first.** Headlines: review-queue formula fixed (missed items now genuinely leave the queue during their spacing window; stale backlog can't bury fresh misses; lapses re-enter at peak−1 per relearning evidence); P0 user_state RMW clobber closed in six API routes (transient read error used to destroy official-exam history/mock flags/saved lists — guardrail test added); session saves deduped + clamped + post-finish answering blocked; mock exit guards + per-question report links; chapter progress pushes milestones immediately with keepalive + sync-failure notice; timezone cookie makes streaks/today/calendars user-local in prod (server-TZ math only ever worked in dev); weekly calendar weekday-anchored (suggestions no longer shift daily; high-band rest day reachable); adaptive plan cold-starts from intake weak areas + guided path instead of 14 rest days; hours slider 3-25 per GMAC medians; review sessions default to study mode; keyboard/SR/contrast/mobile pass (TPA + GI charts now screen-readable; 28 continued GI questions no longer render raw JSON — parser fix + locking test). NOT committed by design: temp QA route (deleted), .env.local. NOTE: `/Users/adam/.claude/launch.json` gained a "GMAT Product Excellence Worktree" entry (port 3311) — the plain "GMAT Platform" entry serves the MAIN repo via --prefix, which earlier sessions' browser QA may not have realized. Deferred/owner items are listed at the end of the report (analytics RPC replay exclusion + tempo definition, TPA attempts column, saved-for-review atomic RPC, DS→DI backfill SQL, /learn hub consolidation, in-reader work → practice_sessions design).

## CONTEXT SWITCH — 2026-07-18 (student-feedback release: verbal wrong-skill pins, question repetition, First 48 Hours + exam roadmap; branch `fix/verbal-selection-onboarding-exam-roadmap`, PR #535 — Adam merges)

Three student complaints diagnosed and fixed off current main (post-#534). NOT merged, NOT deployed. PR #535 has the full summary; highlights + open items:

1. **Verbal wrong-skill questions — root cause was reader pins, not labels.** The 8-stride pin bug (pretests fixed 07-13) survived in recall-check + problem-set pins: 79 slots across 15 verbal chapters re-pinned same-skill/same-tier (all 462 verbal bank labels re-verified accurate by full read; composed tests were already clean). NEW question `critical-reasoning-q249` (Easy Boldface, key C, 2 reviewers) — treat as pre-verified in the QA rotation. Validator ERROR `verbal-topic-noncanonical` + `tests/verbal-chapter-mapping.test.ts` lock the invariant. FLAGGED, not fixed: verbal-02 psets keep basic Strengthen/Weaken items (structure-chapter design call; skill-pure sets need ~6 new Main-Conclusion/Method/Role items); 4 new practice-thin-chapter WARNs (verbal-02/10/11/19 — Boldface/Complete/Attitude/structure banks are thin); index-keyed problemSetRuns mid-run resumes scramble for edited sets (accepted, same as July DS re-pins).
2. **Repetition — `src/lib/question-selection.ts` is now the one sampling policy** (unseen-first seeded, then LRU; RC/MSR passage-aware; small pools fill with LRU repeats + reported counts; constants: 3d recent-seen messaging window, 6h active-deck TTL). Test-builder now history-aware (server passes passage-aware lastSeenAt; UI states forced repeats pre-Build). SessionClient persists deck order to sessionStorage on first submit, restores only TTL-fresh + exact-set-equal (browser-verified end-to-end). Untouched by design: mock composition, chapter tests, review queue, redo-missed. Mixed-review API still Math.random (repetition is its purpose).
3. **First 48 Hours guide** (dashboard, replaces FirstRunGuide; 8 steps auto-derived server-side in `dashboard/first48.ts`, user_metadata persistence, collapses to reopen row; old Getting Started checklists hidden while active) + **exam roadmap** (`deriveExamRoadmap` in `official-exams.ts`, consumed by /mock's Next Recommended Exam panel). GMAC facts verified 2026-07-18 DIRECTLY on mba.com product pages: Exams 1-2 free (Starter Kit), 3-6 paid 2026 bundle, "reset the exam and take each exam twice (in any order)". support.mba.com is Cloudflare-blocked — the old "sets 3&4/5&6 one reset each" framing is STALE, don't reintroduce it; no numeric retake claim for Exams 1-2 is made anywhere (unverifiable from readable primary sources).

Gate at commit: content 0 errors (107 warnings incl. the known deferred one-tier mismatches, now 87), tsc clean, **451 tests** (51 new), eslint (pre-existing scripts warning), build green. Browser-verified via temp public QA routes (deleted): verbal-04/08/17 rendered pins, ch-verbal-05-t1 all-Weaken, deck-restore across hard refresh, builder repeat notice math, guide fresh/partial states, all-six retake roadmap, mobile 375px clean on new surfaces. Pre-existing (NOT this branch): chapter-reader horizontal overflow at 375px reproduces on untouched chapters.

## CONTEXT SWITCH — 2026-07-13 (LATE NIGHT: editorial consistency pass — geometry scoping, DS de-pinning, two-tier fixes, old-scale anchors; UNCOMMITTED pending Adam's review; record appended to GMAT_CONTENT_AUDIT.md)

Adam approved decisions on the content audit's flagged items 1-5. Applied: (1) geometry editorial line adopted — "removed from GMAT Focus Quant; light geometric reasoning can appear in DI contexts" — swept across ~12 files (trig line deleted, formula-sweep geometry removed, master-chapter coordinate-geometry labeled optional/background, DI questions deliberately untouched); (2) all 12 DS-format pins re-pinned out of quant chapters to tier-matched chapter-skill PS items; (3) of the 21 two-tier problem-set mismatches: 8 relabeled Hard→Medium (audit-judged inflated), 7 re-pinned to Beginner same-skill items, 6 flagged unresolved (no same-type Beginner exists — needs new questions or a relabel judgment: CR q5/q20, RC q124/q28/q55/q140); (4) all 49 old-scale "700+/600-level" anchors rewritten scale-free, zero remaining. Script-verified: 0 DS-in-quant pins, 0 "gone entirely" claims, 0 old-scale anchors, exactly the 6 documented two-tier flags. Deferred: 113 one-tier mismatches (pending Hard-tier calibration review), DI geometry syllabus call, audit items 6-10.

## CONTEXT SWITCH — 2026-07-13 (NIGHT: deep product audit — 33 fixes across QA/UX/conversion/SEO/reliability; branch `chore/deep-product-audit`; DEEP_PRODUCT_AUDIT_REPORT.md has everything)

Six-agent audit + fix pass on top of the merged bug-fix release. Read **DEEP_PRODUCT_AUDIT_REPORT.md** for the full list. Headlines: killed the recurring UUID-ordering bug class (dashboard Recent Mistakes + deep-review "last attempt" + a guardrail test banning `.order("id"/"session_id")` repo-wide); TPA questions no longer render answerless on deep review; mock report no longer merges same-day mocks; study-plan's Today's Focus now leads the page; ONE scoring formula everywhere (deleted both remaining local re-implementations); analytics trajectory now question-weighted/review-excluded like the dashboard; dashboard shows the promised "est. NNN" readiness number + an honest trial-status line; last beta-language string killed, "50+ chapters" now flows from site.ts everywhere (12 files); /gmat-free-trial + /gmat-focus-edition-changes de-orphaned via footer; login/signup got real metadata; account deletion now clears email_subscriptions/email_queue/email_events (deleted users kept getting drip emails!); reset-request + lead-capture got in-memory throttles (durable WAF/table = owner task); PAYWALL_ENABLED-without-epoch now logs loudly.

**CONCURRENT SESSION NOTE:** another session was editing `src/content/**` + `scripts/` at the same time and committed its own work as `0b868ed` on this branch (its report: GMAT_CONTENT_AUDIT.md). This session's commit contains none of those files. This session blind-verified samples of that work (3 question rewrites all solve to key; estimated_minutes recalibration defensible; 2 prerequisite fixes justified; ~32 pretest pin swaps structurally green but pedagogically unreviewed) — verdicts in the report. Review both commits before merging the branch.

Gate at commit: content 0 errors, tsc, **400 tests**, eslint (pre-existing script warning), build green.

## CONTEXT SWITCH — 2026-07-13 (ACADEMIC CONTENT AUDIT — chapters/pretests/classification/difficulty/terminology; committed on `chore/deep-product-audit`; read GMAT_CONTENT_AUDIT.md first)

Deep academic audit of course + bank (NOT a key re-audit — keys stay verified per 2026-07-06). **GMAT_CONTENT_AUDIT.md at the repo root is the deliverable**: everything fixed, everything flagged for Adam, in priority order. Fixed this session: 3 true duplicate questions rewritten + blind-verified (algebra q79, arithmetic q47, stats-prob q23); **systematic pretest bug — 24 pretest pins across 19 chapters pointed at the wrong skill** (pins were laid on a mechanical 8-question stride; e.g. the Assumption chapter's "try before you learn" served two Weaken items) — re-pinned to type-matched unpinned questions (safe: reader progress is id-keyed); RC +1 (q178, Attitude, blind-verified 2/2) to keep verbal-19's pool at cap; 9 noncanonical difficulty labels → Hard; GI q38 got a real table (last DI item with no visual); 22 quant chapters' estimated_minutes recomputed (old values implied 360–1,362 wpm and corrupted study-plan day loads); 2 stale prerequisites (quant-05, quant-28); verbal-13 title; lessons/06 mock duration. Validator: +9 rules (unicode-folded dup detection, raw-difficulty canon ERROR, prereq-order ERROR, GI-visual ERROR, DS-in-quant-chapter WARN, pset-tier-mismatch WARN, minutes-implausible WARN, no-pretest/no-summary INFO, old-scale-score-anchor INFO). TOP FLAGGED (Adam's calls, see the audit doc): (1) geometry self-contradiction — "gone entirely" vs DI geometry teaching/bank content + guides/master-chapter still teaching Quant coordinate geometry + a law-of-cosines/trig line in ds-logic-deep-dive that's wrong in any edition; (2) 12 DS items pinned in Quant chapters (validator now warns); (3) 134 pset-tier vs difficulty-label mismatches; (4) Hard tier systematically over-labeled (14/55 sampled a tier high, worst word-problems); (5) ~49 old-scale "700+" anchors; (6) quant-27 teaches its intro twice; (7) CR/RC split disagreement between guides. Gate at commit: content validator 0 errors + full vitest suite green. NOTE: a CONCURRENT session was live-editing app pages (dashboard/mock-report/practice-history/review-question/study-plan-adaptive/login) in this shared tree at commit time with in-flight tsc errors in ITS files — this audit's changeset is content markdown + scripts/validate-content.ts only (scripts/ is excluded from the app tsconfig, so repo tsc/build state is entirely the other session's); committed file-separated, that session's files left untouched, same protocol as 2026-07-07.

Second pass same day (4-agent investigation + verification pass). Fixed: (1) **/study-plan called computeStudyPlan with officialExamCount hardcoded 0** (the "preserved deliberately" comment) — the priority-100 "Set your baseline" card sat permanently on Today's Focus even after the baseline was entered (this was ALSO the friend's "keeps asking for first trial exam"); derivation moved above the Promise.all. (2) **Chapter % stuck at 0**: ChapterReader's flush effect (deps [slug, progress]) cancelled every fresh debounced push and re-sent the STALE snapshot — server was permanently one action behind, single-action visits synced nothing; fixed with a progress ref + [slug] deps. Also: /api/chapter-progress now 503s instead of wiping the whole map when the user_state read errors (readStateRow exported), and dashboard/study-plan now use the /chapters reading-sections completion rule via shared isChapterRead() (the old every-section rule kept "Course progress" at 0%). (3) **Retry fake 0%**: "Practice again" linked to the URL the results screen was mounted on — same-URL nav never remounts, so (pre-freeze) the Date.now()-reshuffled questions were re-scored against stale answers as ~0%; now a client-side restartSession() reset + a new "Redo N missed" wrong-only retry (lib/practice-retry.ts; browser-verified). (4) **Metrics cleanup**: dashboard Accuracy now question-weighted excluding review-* sessions; ONE local day boundary (lib/utils localDayIso) for streaks/dots/hours (UTC slicing put late-evening Oslo sessions on the wrong day); ONE daysUntil() for all exam countdowns (off-by-one); analytics BaselineView's impossible 0/N checklist reduced to the baseline row; badges count chapter reads (not just deprecated lesson_completions); dashboard mistake chip = not-reviewed definition ("N to review"); study-plan "Projected total" card DELETED (3rd divergent score estimate on an uncapped ~1000-row-truncated query — /analytics owns the estimate). Onboarding copy no longer implies the wizard's prior-score field is the baseline. **Owner SQL (one-liner, in BUGFIX_REPORT):** backfill `practice_attempts.section='DI' where question_type='Data Sufficiency'` to heal the DS→DI history split. NOTE: live `practice_attempts` HAS a created_at column — the schema snapshot at the bottom of this file is stale on that point. Gate: content 0 errors, tsc, 399 tests, eslint (pre-existing warning), build green.

## CONTEXT SWITCH — 2026-07-13 (EARLIER: 11 student-facing study/practice/review bugs fixed end-to-end; full details in BUGFIX_REPORT.md — read that first)

Adam supplied an 11-item student bug list (answer reveal, save-for-review, mark-reviewed, DS-in-Quant, error-log order, custom-set results history, ch.16 empty pretest, timer during solutions, content-after-summary, graded-set resume, study-hours cap). All 11 fixed; **BUGFIX_REPORT.md at the repo root** has per-bug root cause / fix / tests / decisions — keep it until Adam reviews, then it can be deleted (it was explicitly requested, not doc-noise).

Three root causes worth remembering: (1) **`router.refresh()` + Date.now()-seeded adaptive reorder desynced the mounted SessionClient** (answers/reveals landed on the wrong questions) — fixed by freezing the question list per mount; (2) **`staleTimes.dynamic: 30` in next.config served pre-write RSC payloads on quick back-nav**, making persisted saved/reviewed toggles render unchecked — removed (dynamic back to 0); (3) **125 DS-typed questions in quant banks inherited `section: Quant`** — parser now forces DS → DI (Quant pool 928/0 DS, DI 596/225 DS; old attempt rows re-derive display section in the error log only). Also new: `/practice/history/[id]` saved-results page (test-builder recent builds + results screen link to it); graded-set mid-run resume via `chapter_progress.problemSetRuns` (rides existing localStorage+server push; merge keeps the longer run); session timer now counts answering time only (`src/lib/practice-timing.ts`) everywhere incl. persisted total_time_ms; onboarding weekly-hours slider 1–40 with band advice and the value now actually shapes buildWeeklyCadence (weak-areas-first when low, rest day when high); two new validator ERRORs (`chapter-empty-pretest`, `chapter-content-after-summary`); empty pretests removed from quant-04/quant-30 (populating = authorial call, flagged in the report); di-2's `the-ds-method` moved before summary.

Gate: content 0 errors, tsc clean, **391 tests** (12 new), eslint (pre-existing script warning only), build green. Browser-verified via a temp public route (deleted): pre-submit reveal gating + timer freeze/resume (clock pinned 31s wall while reading a solution; results showed frozen total). Auth-gated flows verified at unit/loader level — no local creds, and account creation is out of bounds for me.

## CONTEXT SWITCH — 2026-07-13 (acquisition cleanup: ONE public offer everywhere; new /gmat-free-trial page; branch `copy/acquisition-cleanup`, pushed — Adam merges)

Full public-copy audit against the Deep Research report ("deep-research-report (2).md"). The single public offer is now consistent on every cold surface: **free 7-day full-access trial, no card, then a one-time paid plan**. Full findings table + final approved offer wording + remaining owner decisions live in `marketing-drafts/acquisition-cleanup/OFFER_AUDIT.md` — read that first before touching marketing copy.

Headlines: (1) **NEW `/gmat-free-trial`** (AcquisitionLanding, statically prerendered) is the clean bottom-funnel page for commercial-intent/paid traffic; `/gmat-private-beta` page DELETED and 308-redirects to it (next.config.ts), sitemap swapped. (2) **FoundingOffer removed from the homepage** — founding is now down-funnel only (/pricing `#founding`, ReserveInterceptButton reveal, day-5/8 drip emails, referral mechanics, warm retargeting ad copy); all founding wording moved from "when paid plans launch" to "when checkout opens" (plans are visible; only checkout is pending). (3) Pricing banner now states plainly that checkout hasn't opened and prices are one-time. (4) PricingCard excluded features get an sr-only "Not included:" prefix (scrapers/report read Self-Study as including WhatsApp/coaching). (5) Four landing pages' lead blocks promised emailed assets the `newsletter` magnet never delivers (study-plan "worksheet", quant "sample chapter", DI "checklist", mock-review "checklist") — all switched to the real error-log-template download. (6) Error-log promos say "no account — just your email" (email IS required); the public CSV's own CTA row de-beta'd. (7) AD_COPY_BANK + google-ads-launch/* (ADS.csv CSV-parse-verified, KEYWORDS, LANDING_PAGE_MAP, CAMPAIGN_STRUCTURE, etc.) reframed to trial language, prep-platform ad group → /gmat-free-trial optimizing to `signup`; the old GOOGLE_ADS_SETUP.md got a STALE banner. Other marketing-drafts NOT swept (no live ads on those channels) — see OFFER_AUDIT risk list.

Gate at commit: content 0 errors, tsc clean, 379 tests, eslint (1 pre-existing warning in scripts/send-consult-batch.ts), build green; all main public pages curl-200, redirect 308 verified, rendered-HTML claim assertions pass. **First ad traffic should go to /gmat-free-trial (commercial) + /gmat-study-plan (planning) — but only after the GA4→Ads conversion import (owner console task) is done; ads stay paused until then.**

## STATUS — 2026-07-07: the 2026-07-06 content-audit + growth work below is now COMMITTED

All of this session's work (grow thin banks + full-product QA audit, the seven `2026-07-06` entries below) is committed — the "UNCOMMITTED" labels in those entries are now historical. **Committed as 3 commits (`4904981` grow banks, `b8d75ea` QA-audit fixes, + this doc) on branch `outreach/signup-consult-email`.** NOTE: that branch belongs to a *concurrent* email-outreach session; the working tree was on it (shared tree) so the commits landed there to avoid disrupting that session. **Adam: these 3 content commits are cleanly file-separated — cherry-pick them onto `main` (or a fresh `content/qa-audit` branch) rather than merging the whole outreach branch, unless you want the outreach work too.** The concurrent session's own outreach commits + one untracked `scripts/send-test-consult.ts` are NOT mine and were left alone. Gate green at commit: content 0 errors, 379 tests, eslint, build.

**What this session did (all committed):** grew RC +24 / arithmetic +6 / combinatorics +16 (cleared all 8 thin-chapter warnings) + fixed a Fraction-Operations routing collision; then audited EVERY content surface (all 20 question banks ~1,720 Qs + all quant/DI worked examples ~600+ + all 34 verbal teaching files) and fixed **18 content defects + a 6-question rendering bug** (DS Q44, TA Q17/18/19/23, algebra Q95, backsolving Q39/Q40, estimation-layout×6, CR Q157/Q221/Q200, quant-number-properties, quant-14, quant-22, di-6, reading-verbal-05/07). One open author-judgment item: **reading-verbal-08 elite-habits checkpoint** (a scenario/label doesn't match the five taught habits).

**STILL OPEN in this handoff — NOT touched this session (older threads, mostly owner-only):**
- *Owner-only / external (cannot be done in-repo):* Google Ads conversion-action import (audit's #1 gap) + auto-apply off; delete the test account; Google Search Console index request; Stripe live activation — the last is also blocked on the processor decision.
- *Owner strategic decisions:* the processor pivot (Armenian IE + merchant-of-record) and accountant questions; whether to remove the "free while in beta" banner on /pricing; the paywall flip (blocked on processor, not code).
- *Marketing / code follow-ups (a future session COULD do):* the 4 newsletter AcquisitionLanding pages still promise assets that don't exist (study-plan worksheet / DS cheat sheet / Verbal worksheet / one-pager) — flagged, not fixed; the 0% marketing-opt-in "return loop" (CTA in the error-log CSV + download screen, opt-in copy rework); in-app referral prompts (post-signup/post-value); `grant-access.ts --revoke`, an `/admin` lookup UI, the N-month access-cutoff enforcement.
- *Note:* the older 2026-07-01→07-03 "UNCOMMITTED" changesets described further below are ALREADY shipped (merged as PRs #517–#520) — do not re-do them.

## CONTEXT SWITCH — 2026-07-06 (FINAL++: VERBAL teaching content audited — the LAST content surface; 1 fix + 2 flagged; UNCOMMITTED, same tree)

Audited the remaining surface: the **CR/RC teaching chapters & guides** (34 files, worked examples + embedded arguments/passages), scoped to concrete correctness only (not pedagogy). Verbal prose was very clean (1 concrete error in 34 files).
- **FIXED — [guides/reading-verbal-05-rc-mindset.md](src/content/guides/reading-verbal-05-rc-mindset.md)**: a recall-check answer's time breakdown ("60-90 sec for first read + 60-90 sec/question × 4") didn't sum to its stated "8 minutes" and contradicted the chapter's own taught budget (~2 min first read); changed to "~2 min for the first read + 60-90 sec per question × 4".
- **FIXED (proposed rewrite) — reading-verbal-07-trap-answer-patterns.md**: the free-shipping CR example conflated premise ("customer return rates") with conclusion ("spend more"). Made it consistent about spending — premise → "higher average customer spending," choice D → "average customer spending dropped sharply." Verified: 4/4 blind solvers pick D with exactly one strengthener. Determined fix (matches the conclusion), applied.
- **STILL FLAGGED (design call — not fixed): reading-verbal-08-elite-solvers.md** — the Application checkpoint lists "fatigue-resistant habit deployment" as a "five elite habits" option, but that isn't one of the five taught (eliminating-with-discipline is missing from the list), and one application scenario is about fatigue with no matching taught habit. Reconciling scenarios↔labels↔habits needs your design intent, so left for you.
- **CR Q200 also now FIXED (proposed):** see the note above — its keyed option C didn't actually weaken; replaced with a disanalogy weakener (to-go customers don't return the tumblers), 4/4 blind-verified to C. So of the earlier "flagged for Adam" items only reading-verbal-08 remains open.
- **With this, EVERY content surface in the product is audited: all question banks, all quant/DI worked examples, and all verbal teaching content.**

## CONTEXT SWITCH — 2026-07-06 (FINAL+: CHAPTER teaching content audited too — worked-example arithmetic across 48 chapters/guides; 4 real defects FIXED; UNCOMMITTED, same tree)

After auditing every QUESTION bank (below), audited the remaining computational surface: the **worked examples / drills inside the chapter readings & guides** (~600+ examples, 48-agent fan-out, one per chapter). **4 real arithmetic errors found + fixed, 1 false positive dismissed:**
1. **[guides/quant-number-properties.md](src/content/guides/quant-number-properties.md)** — units-digit example: 7^23 units is 3 not 7 (23 mod 4 = 3 → 3rd in cycle [7,9,3,1]); so 3^47+7^23 = …7+…3 = …0, not "…7+…7=14 → 4". Fixed answer 4→0.
2. **[chapters/quant-14-quadratics-factoring.md](src/content/chapters/quant-14-quadratics-factoring.md)** — Vieta example claimed xy=6, x²+y²=20 are satisfied by "x=3, y=2" (3²+2²=13≠20); fixed to the true irrational values x=3√2, y=√2.
3. **[chapters/quant-22-work-rate.md](src/content/chapters/quant-22-work-rate.md)** — garbled speed-comparison worked example (wrong factorization (r−40)(r+60), r=40 didn't satisfy r²+10r−2400, and a paragraph admitting the numbers don't work). Rewrote to the clean 18-minute version: 60/r−60/(r+10)=3/10 → r²+10r−2000=0 → (r−40)(r+50)=0 → r=40 (verified 60/40−60/50=18 min).
4. **[chapters/di-6-multi-source-reasoning.md](src/content/chapters/di-6-multi-source-reasoning.md)** — "10.93/24.30 = 44.97%, misses by 0.03pp" → correct 44.98%, misses by 0.02pp (ironically inside a "precision matters" example).
- False positive left alone: quant-28's "3q=54 → q=22 … recheck … q=18" is deliberate error-then-self-correction pedagogy (final answer q=18 is right).
- Each fix independently re-verified in Python; gate green. **With this, EVERY computational content surface in the product — question banks AND chapter teaching content — has been audited this session.**

## CONTEXT SWITCH — 2026-07-06 (FINAL: ENTIRE QUESTION BANK audited — 1,720 questions across all 20 banks re-solved blind; 9 content defects + 1 six-question rendering bug FIXED, 2 CR items flagged; UNCOMMITTED, same tree)

Final piece: audited the verbal banks (**critical-reasoning 246 + reading-comprehension Passages 4–37 / 141 = 387 Qs**) — verbal-judged (blind re-solve → letter reconcile → 3-judge adversarial panels on every mismatch/flag; no python ground truth). **RC 141/141 clean. CR 241/246 agreed; all 5 mismatches went to panels and every one UPHELD the published key** (the blind picks were hard distractors). Three low-confidence flags questioned key coherence → coherence panels:
- **CR Q157 (Paradox) — FIXED** (3/3 unanimous defect): keyed answer A explained the higher electric bill by "air-conditioning ran much longer" because LEDs give off less heat — but less internal heat means LESS cooling, so AC runs LESS. Physically inverted. Fixed to "electric heating systems ran much longer" (+ explanation): losing the incandescent bulbs' incidental heat forces the heaters to compensate — a valid paradox. File: [critical-reasoning.md](src/content/questions/verbal/critical-reasoning.md).
- **CR Q221 (Weaken) — FIXED (proposed rewrite, review)** (3/3 unanimous defect): keyed B was self-contradictory — if the 20% revenue rise "came from existing indoor customers who moved to the patio... rather than adding new diners," revenue couldn't have risen at all. Replaced B with a clean alternative-cause weakener: "The 20 percent revenue increase was driven mainly by a citywide surge in summer tourism, which also lifted revenue at nearby restaurants that have no outdoor patio" (+ explanation rewritten). Verified: 4/4 independent blind solvers pick B, all reporting exactly one clear weakener. Answer unchanged (B). **This is a proposed rewrite — reword to your voice or revert if you prefer.**
- **CR Q200 (Weaken) — FIXED (proposed rewrite)**: the coherence panel and a closer look confirmed the old option C ("nearly all customers are to-go") does NOT weaken — if anything, a larger to-go group means MORE waste to cut. Replaced C with a disanalogy weakener: "To-go customers rarely bring the steel tumblers back, so the café would have to keep buying and discarding replacement tumblers" (the projected savings depend on reuse, which to-go customers won't do — unlike in-store mugs). Rewrote the explanation; answer stays C; 4/4 blind solvers pick C with exactly one weakener. Applied uncommitted.
- Fix-vs-flag line held all session: mechanical corrections and panel-verified rewrites applied (uncommitted); genuinely contested items flagged.
- **With this, EVERY question bank in the product is fully audited (all 20 banks, ~1,720 Qs this session). Total: 10 content defects fixed [incl. Q221 proposed] + 1 rendering bug fixed + 1 CR item (Q200) flagged for you.**

## CONTEXT SWITCH — 2026-07-06 (earlier: QA audit over all 12 quant + all 5 DI banks — 1,333 questions re-solved blind; 8 answer/content defects + 1 six-question RENDERING BUG found, ALL FIXED; UNCOMMITTED, same tree)

Extends the entry below: a final 69-agent sweep covered the 8 remaining quant banks (backsolving/estimation/plugging-in/rates-work/word-problems/ratios-percents/algebra/combinatorics, 648 Qs; blind agree 645/648). Three more real defects + one production rendering bug, all fixed (files: [algebra.md](src/content/questions/quant/algebra.md), [backsolving.md](src/content/questions/quant/backsolving.md), [estimation.md](src/content/questions/quant/estimation.md)):
1. **algebra Q95 — WRONG KEY**: stored D(47) while the explanation itself derives n=45 → largest = 49 = E. Key → E, trap note added.
2. **backsolving Q39 + Q40 — contradictory DS statements** (same malformed class as DS Q44): Q39's statements gave d=15 vs d=8 (St.1 total $1.95→$1.60; both now give 8; answer D unchanged); Q40's St.2 forced n=11, not a factor of 12 (St.2 → "prime greater than 2 and less than 5", n=3; answer B unchanged).
3. **RENDERING BUG — estimation Q20/22/23/33/34/35**: their DS statements sat AFTER the options, and content.ts ends the prompt at the first option line, so all six rendered STATEMENT-LESS in the live app (unanswerable; inverse sibling of the 07-03 TPA prompt-leak). Content-only fix: statements moved above the options in all six; then all six keys were blind-re-solved (genuinely tested for the first time) — all correct. If new DS content is authored, keep statements BEFORE options.
- Also verified clean this round: 25 selection/property questions across the new banks (tie-hunt analog, all unique); word-problems Q51 (flagged "duplicate answer" was a tooling artifact — bank clean).
- **Bottom line for the whole session's audit: every quant and DI bank is now fully audited (1,333 Qs). Total defects: DS Q44, TA Q17/18/19/23, algebra Q95, backsolving Q39/Q40, + the 6-question estimation rendering bug. Only verbal CR Q16–119/158–246 and RC Passages 4–37 remain un-audited.** Gate green after everything: content 0 errors, 379 tests, eslint, build.

## CONTEXT SWITCH — 2026-07-06 (earlier: QA-audit sweep — 685 questions across 7 whole banks re-solved blind, 5 real defects found + FIXED [DS Q44 + table-analysis Q17/18/19/23]; ALL FIVE DI BANKS NOW FULLY AUDITED; UNCOMMITTED, same tree)

After the content-growth work below, ran a deep QA-audit rotation with a blind-resolve + brute-force-python harness (details + reusable scripts in the `reference_gmat_qa_audit_log` memory). Swept **7 whole banks, 685 questions**: quant/number-properties (100), quant/exponents-roots (88), quant/statistics-probability (85), quant/arithmetic (124), di/two-part-analysis (95 — 75 quantitative pairs confirmed by exact-fraction python + 20 verbal picks matched blind; 2 near-tie flags adjudicated by skeptics as legitimate hard distractors) — all **0 defects**; di/data-sufficiency (100) — **1 defect**; di/table-analysis (93) — **4 defects**. **All 5 defects are in DS + TA and all are fixed (uncommitted).** With this, every DI bank is fully audited (GI + MSR were done 06-22). Two files changed by the audit: **[data-sufficiency.md](src/content/questions/di/data-sufficiency.md)** and **[table-analysis.md](src/content/questions/di/table-analysis.md)**.
- **DS Q44** (published **E**, now **C**): malformed — "circle center (h,k) r; is (3,4) inside? (1) passes through origin (2) h=0,k=0" forces r=0, contradicting the "circle has positive radius" stem; E's "degenerate→can't-determine" is a category error. Confirmed by 5 independent analyses (blind solver + python + 3 adversarial skeptics, unanimous). Fixed by changing (1) to "passes through (6, 8)" → r=10, (3,4) inside → clean C; explanation rewritten.
- **table-analysis Q17, Q18, Q19, Q23** — all the ambiguous-multiple-correct / tied-extreme class, and every one's own explanation CONCEDED the tie (e.g. Q18 "Bravo, Charlie, and Echo each produce a density of exactly 9.0… Among those three, Echo is the credited selection"). Two were caught by the reconciler (Q18 three-way density tie at 9.0; Q19 two firms tied at R&D ratio 1/4); the other two only by a follow-up **tie-hunt** over all 46 superlative questions (Q17 Helios=Meridian=80%; Q23 Glenwood=Ivyton=+60) because the solver's arbitrary tie-break had matched the stored key. Each fixed by nudging ONE data cell of the *non-credited* tied option so the credited answer is the unique extreme; explanations rewritten; credited letters unchanged (E,E,A,B). A deterministic re-check of all 15 quant selection questions found 0 ties (the class is specific to table-analysis's compute-5-from-a-table shape).
- **Adam: review the 5 fixes** (esp. DS Q44 — you may prefer to retire the trap). The other 585 audited questions were confirmed correct two independent ways.
- **DI rotation COMPLETE** — all five DI banks fully audited (two-part-analysis closed out at 95/95 clean, incl. skeptic adjudication of its 2 near-tie flags and refutation of a false "leaked header" defect claim that was the audit parser's own artifact). Remaining un-audited, by yield: verbal CR Q16–119/Q158–246 + RC Passages 4–37 (biggest untouched mass, verbal-judged), then the small quant banks (backsolving/estimation/plugging-in) and partial ranges (rates-work/word-problems/ratios-percents/algebra/combinatorics). TWO KEY LESSONS logged in the QA memory: (1) for any "which is highest/closest/most X" bank the disagreement reconciler is NOT enough — also tie-hunt (exact metric per option, flag when >1 achieves the extreme); (2) re-check agent "defect" claims against the raw markdown before crediting them — parser artifacts masquerade as bank defects.

## CONTEXT SWITCH — 2026-07-06 (LATER: grew the QUANT banks +22 questions — all 4 quant thin chapters cleared + a pre-existing routing bug fixed; UNCOMMITTED, same tree as the RC entry below)

Continued from the RC session below (all 8 `practice-thin-chapter` warnings are now **0**). Added **22 new Problem-Solving questions** to clear the four thin quant chapters: **arithmetic.md Q119–124** (6, all → quant-06-fractions-decimals) and **combinatorics.md Q105–120** (16 → quant-24/25/26 by subtopic routing). Bank Quant 1031→1053, whole bank ~1,957→1,979.
- **Test-pool result** (cap 15, all cleared with headroom, 2 tests each): quant-06-fractions-decimals 10→16, quant-24-counting-basics 12→16, quant-25-permutations-combinations 13→17, quant-26-restrictions-advanced-counting 8→16.
- **Correctness — two independent proofs per question.** (1) each generator emitted a self-contained Python check that recomputes the answer FROM FIRST PRINCIPLES — brute-force `itertools` enumeration for every counting question (e.g. Q118 enumerates all 6! permutations with A-before-B and C,D adjacent = 120; Q120 enumerates C(11,5) committees with ≥2M/≥2W = 350), exact `fractions.Fraction` for the arithmetic — and I RAN all 22 as a gate: computed value == claimed answer == marked option, options ascending+distinct. (2) an independent BLIND agent re-solved each from scratch (no key) and had to pick the same letter. **22/22 pass both.** One fix during gating: Q122's options weren't ascending (5/11 vs 9/20) — swapped A↔B, answer→B, re-passed.
- **Routing bug found + root-caused.** My Q123 (subtopic "Fraction Operations") mis-routed to quant-18-ratios-proportions because the arithmetic router matches the substring "ratio" and it hides inside "ope**ratio**ns". This also affected **3 pre-existing questions** (arithmetic Q at lines ~1825/1882/1901 — a fraction-division chain, a 7/40→decimal/percent conversion, a continued fraction — all genuinely fraction problems). Root-cause fix: relabeled all 4 `**topic:** Fraction Operations` → `Fractions` (collision-free, routes to quant-06). quant-18 stayed healthy (25→24 test pool). **Latent gotcha for future authoring: the router uses plain substring matching, so any subtopic containing a hidden keyword mis-routes — "operations"⊃"ratio", watch for others.**
- **Answer-letter spread** A1/B7/C5/D7/E2 — left as-is: quant options are sorted ascending (letter = the answer's rank), so mid-clustering (B/C/D) with rare extremes is the realistic GMAT pattern, unlike the RC case which needed rebalancing.
- **Format**: rendered deterministically to match the bank byte-for-byte (`## Qn`, core fields only — no hint_/mistake_ fields, matching the arithmetic convention; `**related_reading:**` per bank). Gate green: content **0 errors** (12 warnings — the 8 thin-chapter warnings all gone, remaining are pre-existing no-mental-model/no-trap-callouts/thin-examples/duplicate-prompt/default-routed), tsc, **379 tests**, eslint, `next build`.
- Files: arithmetic.md, combinatorics.md (both UNCOMMITTED alongside the RC file). **Remaining thin: none.** The only content-warnings left are authorial chapter-reader pedagogy (no-mental-model ×7, no-trap-callouts ×5, thin-examples ×7) — real polish but they mean writing teaching callouts into Adam's chapters, a different (authorial) kind of task. Adam's call whether to have me draft those. UNCOMMITTED; Adam reviews.

## CONTEXT SWITCH — 2026-07-06 (grew the RC bank +24 questions to clear the four thin RC chapters flagged below; UNCOMMITTED)

Closed the "grow the RC bank (Main Idea / Specific Detail passages) when convenient" follow-up flagged in the two 2026-07-03 entries — verbal-14-rc-main-idea and verbal-15-rc-detail were sitting at the MIN_TEST_POOL=4 floor (fragile: any future reader-pin would drop them below the test minimum). Added **6 new RC passages (Passage 38–43, Q154–Q177 = 24 questions)** to `src/content/questions/verbal/reading-comprehension.md`; bank RC 153→177, Verbal total 431→455, whole bank ~1,933→1,957 (QUESTION_CLAIM "1,900+" still true — it's a floor).
- **Test-pool result** (all four thin RC chapters cleared, now 2 tests each with headroom over the Verbal cap of 8): verbal-14-rc-main-idea 4→10, verbal-15-rc-detail 4→10, verbal-17-rc-application 6→10, verbal-20-rc-answer-traps 6→10. Topic mix authored to feed exactly these: 6 Main Idea, 6 Specific Detail, 4 Application, 4 Answer Traps, + 1 each Inference/Function/Passage Structure/Author's Attitude. New questions aren't reader-pinned, so each lands straight in its chapter's test pool.
- **How**: two verified workflows. (1) generate — one agent per passage authored a full passage + 4 questions, then an independent BLIND verifier re-solved every question (no key shown) to confirm a single defensible answer; 0 flags survived. (2) polish — rebalanced answer letters (they'd clustered on B, 14/24; now even **A5 B5 C5 D5 E4**, each passage 4 distinct letters), renamed 3 passages that had independently converged on "…Ledger", fixed one typo ("retddailer"), then blind-re-solved all 24 again. The repositioning introduced 2 duplicate-option defects (P40 Q164 / P41 Q167 had options B==E) — caught by the re-verify, fixed surgically (restored the displaced distractor into B), and blind-re-verified clean (both uniquely answer E).
- **Format**: rendered deterministically from structured JSON so bank markdown is byte-exact (`## Passage N: Title` → `### Qn` grouped, difficulty Easy/Medium/Hard, 5 distinct options, `**answer:**`/`**explanation:**`/`**mistake_x:**`/`**related_reading:** reading-verbal-06-rc-question-types`). Every question's mistake field for its own answer letter is empty; explanation ends "The correct answer is X."
- **Gate green**: content **0 errors** (16 warnings, was 19 — 4 RC thin cleared; the one added `duplicate-prompt` group is the 6 Main Idea questions sharing the standard "The primary purpose of the passage is to" stem, which the bank already reuses across 11 questions — benign, matches convention), tsc clean, **379 tests** (bank-wide parse-integrity + embed-uniqueness + test-composition all run over the new questions and pass), eslint clean, `next build` clean.
- **Still thin (out of scope — flag to Adam)**: 4 QUANT thin-chapter warnings remain (quant-06-fractions-decimals 10, quant-24-counting-basics 12, quant-25-permutations-combinations 13, quant-26-restrictions-advanced-counting 8) — same "grow the bank" fix, different section. Not browser-verified (auth-gated readers, no local creds — same constraint as prior content entries); correctness is at the content/loader layer, covered by the blind verifiers + the gate. UNCOMMITTED; Adam reviews.
- **Note**: this HANDOFF was behind git — the two 07-03 entries below were committed/merged (PRs #517/#518), and two later sessions merged after them without adding entries here (PR #519 AdSense meta, PR #520 founder-voice signup drip). Main is at those; this RC work is uncommitted on top.

## CONTEXT SWITCH — 2026-07-03 (LATER: beta-feedback sweep — 11 fixes from two testers' feedback; UNCOMMITTED)

Adam supplied a dated feedback file from a friend (13/06–03/07 with screenshots) + a second tester's "hard to tell where to start" note, and said "fix ALL". Diagnosed by a 5-agent read-only workflow (every finding CONFIRMED with file anchors), then fixed. Gate green: content 0 errors, tsc, **379 tests**, eslint, build. Browser-verified with a disposable prod user (deleted after): onboarding step, TPA de-spoiler, timer freeze, future-date 400.

1. **TPA solution leak (worst find)** — parseQuestionBlock's prompt region only ended at "- A)" option lines; TPA questions have none (pipe-table options), so ALL 95 two-part-analysis prompts shipped their full worked explanation + "the correct answers are…" line on every surface. Fixed in content.ts (prompt also ends at the first tail-field marker). Verified: 0 leaks bank-wide, 95/95 TPA structurally intact. Locked by tests/question-parse-integrity.test.ts.
2. **Practice timer never stopped at the end** — results "Total time" was a live clock, and a late "Retry save" inflated stored total_time_ms by idle minutes. Fixed: sessionEndedAt captured at both finish sites; results/header clocks + save payload read the frozen instant; the 1s tick stops on results. Verified live (frozen at 00:43). MockRunner checked — already correct.
3. **"Mark as reviewed" / "Save for review" vanishing on window change** — three causes: SessionClient remounted the save button per question with initialSaved={false} (now seeded from user_state via initialSavedForReview threaded through [slug]/custom/review pages + a savedIds set); SaveForReviewButton + ErrorLogClient never resynced to fresh server props and never router.refresh()ed after writes vs the staleTimes.dynamic Router Cache (both now prop-resync + refresh-on-success; cycleRemediation too). Stale user_metadata docstrings corrected to user_state.
4. **Chapters skipped the pretest** — /chapters resume anchors were computed over reading-type sections only and emitted even for never-opened chapters, deep-linking past "try before you learn". Fixed: firstUnread over ALL sections; anchor only when started; isStarted counts pretest-only progress.
5. **MixedReviewCard unreadable in the reader's light theme** — hardcoded dark-palette text on cream; every color now var(--read-*, dark-fallback) — dark surfaces pixel-identical, reader themes resolve.
6. **Future-dated official scores** (friend scored a Nov-1 slot 705 in July; trend read "on target") — /api/official-exams rejects dates beyond today+1d UTC grace (400, verified live) AND the /mock EntryForm date input gets max=todayIso + a submit check. Existing polluted rows are user-deletable via the row's trash button.
7. **Onboarding exam-date "skip" removed** (it only cleared the field — read as broken; Adam's explicit ask); replaced with "Leave it empty and hit Next."
8. **Radical readability** — all ∛/∜ glyphs in the bank rewritten to caret fractional-exponent form (renders as real superscripts via rehype-caret-sup) incl. the reported (−27)^(1/3); coefficient-radical interpuncts (5x⁴·√3); guide caret-converted; chapter quant-12 keeps radical notation (it teaches it) with the index named on first use.
9. **Worked-example duplicates de-duplicated by number mutation** (a student met the same question up to 4x: drill + reading example + recall check + review): quant-11 sci-notation 0.00045→0.00062; quant-10 factors 720→540 (example + micro-drill lockstep) and remainder 9/5→8/3; quant-19 30%/45%→20%/35% (4 sites); quant-12 √12+√27→√18+√50 (2 sites); quant-17 bakery $21/$17→$24/$18 (scone $6) and consecutive-evens 18→24 (largest 22); quant-01 backsolve sum 78→84; guides/master-chapter CR breakfast→after-school-tutoring scenario. All rewritten arithmetic re-verified. (guides/quant-master-chapter machines example was NOT a duplicate — left alone.)
10. **"Where do I start" (second tester)** — Today's Mission (dashboard hero) no longer vanishes for cold-start users: falls back to "Continue: <chapter>" (resume target), then "Start the course" (first guided chapter); QuickActions is the last resort. Complements FirstRunGuide. (An adaptive placement diagnostic was explicitly NOT built — it would reverse the no-fake-diagnostic decision; flagged to Adam instead.)
11. Verified already-fixed items from the feedback file (no action needed): reader DI charts, cross-chapter reader duplicates (re-verified ZERO overlaps against current content), mark-as-read crash (fixed 06-14, commit 56c09d5).

## CONTEXT SWITCH — 2026-07-03 (reader question repetition fixed for ALL chapters; UNCOMMITTED on the same working tree)

Adam: "the repetitive thing about the questions is still there, fix it for ALL CHAPTERS." The 2026-07-02 fixes below covered topic-drill ordering and reader-vs-test overlap; the remaining repetition was the READER PINS themselves: **250 duplicate pins across 50 chapters** — the same question pinned twice inside one chapter (pretest question reappearing as that chapter's recall check; check questions reappearing in its problem sets, e.g. quant-11 re-asked exponents-roots-q1 minutes after the pretest) plus cross-chapter re-pins (adjacent chapters sharing up to 8; verbal-21-mixed-timing/di-timing-mixed re-serving up to 19 earlier questions).
- **Semantics chosen**: every reader slot (pretest / check / problem set) pins a DISTINCT question across ALL chapters, coming-soon chapters included (hidden only from test routing; their readers are visible). First occurrence in path order keeps the question; every later occurrence was replaced with an unused same-bank question. Consequence: the timing/mixed chapters now drill FRESH questions instead of recapping earlier ones — flag to Adam if recap-by-design is preferred there.
- **How**: 100-agent workflow — one propose-agent per chapter read the section bodies and picked ranked same-type replacements (GI only with chartSpec/context; RC prefers same-passage; problem-set tiers anchored to the outgoing difficulty), deterministic global reconciliation prevented two chapters picking the same question, one apply-agent per chapter edited only its frontmatter. 3 manual corrections after adversarial re-verification: combinatorics-q20→q38 (fallback pick was DS-type for a PS slot), and 2 swaps in verbal-21-mixed-timing (q67→q74, q68→q57) because the originals sat in verbal-14/verbal-15's test pools and would have dropped them below MIN_TEST_POOL=4, re-triggering the reader-in-test overlap via the fallback.
- **Guardrails**: `scripts/validate-content.ts` new ERROR rule `reader-duplicate-pin` (any question pinned in 2+ reader slots anywhere) + `tests/chapter-embed-uniqueness.test.ts` over the real bank.
- **Verified**: audit 0/0/0/0 (within-chapter dups, cross-chapter reader shares, reader-ids-in-tests, cross-test dups); diff is surgical — 50 chapter files, exactly 250 pin lines changed, zero body/format changes; every replacement same-bank + same-type + previously-unpinned + used exactly once. Gate green: content **0 errors** (19 warnings), tsc, **377 tests**, eslint, `next build`.
- **Side effects**: test compositions shift again (reader-exclusion set grew ~516→766; beta-acceptable per the 07-02 precedent). `practice-thin-chapter` warnings now 8 — verbal-14-rc-main-idea and verbal-15-rc-detail sit at exactly the 4-question test-pool minimum; grow the RC bank (Main Idea / Specific Detail passages) when convenient.
- Not browser-verified (auth-gated readers, no local test credentials — same constraint as the 07-02 TPA entry); correctness is at the content/loader layer, which the audit + tests cover. UNCOMMITTED; Adam reviews.

## STRATEGY UPDATE — 2026-07-02 (ENTITY/PROCESSOR: Stripe-via-Norway is DEAD; leading path = Armenian IE + merchant of record)

New facts from Adam: he holds a RUSSIAN passport; his Norwegian residence permit (self-employment forbidden) expires 2026-08-31; from September he's on a Swiss STUDENT permit (also no self-employment); his parents live in Armenia; his uncle is a Swiss citizen in Switzerland. Consequences for the go-live plan below:
- **The "complete Stripe activation under Norway" step is NON-VIABLE** — Stripe Norway only accepts registered businesses, and Adam cannot legally register one on his permit (nor in the 8 weeks remaining). Do not chase it.
- **US LLC (Atlas) is hostile** on a Russian passport (Mercury bars RU citizens regardless of residence; Wise conflicting).
- **Leading path: Armenian Individual Entrepreneur** — foreign citizens register without residence permits, remotely via notarized POA (parents on the ground), ~AMD 3,000 duty, 1% turnover tax via High-Tech Registry through 2031. Pair with a MERCHANT OF RECORD (Paddle or Lemon Squeezy — both accept Armenian sellers, and MoR absorbs global VAT). The closed PR #512 contains a Paddle mapping prototype; entitlements/trial core is processor-agnostic by design. Expect a focused checkout-pivot session once the IE + MoR account exist.
- Swiss GmbH (uncle as resident director) is the LATER scale-up option — CHF 20k + admin, not the launch vehicle.
- Owner to settle with an accountant: NO/CH personal tax on IE income; student-permit gray zone for actively running a foreign business; High-Tech Registry qualification (90% IT activity) for an edtech SaaS; election timing for new registrants.
- Until this lands, the paywall flip stays blocked on the processor, NOT on code. Everything else in the flip runbook (epoch, copy sweep, founding codes) is unchanged.
- **Interim purchase-intent fix (UNCOMMITTED, same tree, iterated with Adam)**: /pricing was showing LIVE buy buttons wired to TEST-mode Stripe — an ad-driven stranger clicking "Get Self-Study" reached a checkout that declines real cards (trust-killing dead end at peak intent). Final design per Adam: the cards keep their normal buy CTAs ("Get Self-Study" etc. — the page still reads as a pricing page), and only ON CLICK does `ReserveInterceptButton` (new component) reveal the interim story: checkout opens shortly, platform free meanwhile, "Reserve my founding rate" -> #founding block. The click fires `checkout_initiated` with placement "pre-checkout", so true purchase intent during the processor gap is measurable per plan. Wired via PricingCard's new `purchasable` prop (= PAYWALL_ENABLED on /pricing; /upgrade keeps real CheckoutButton). Reverts to live checkout automatically at flip. Browser-verified: 4 normal CTAs, no free-mention by default, click reveals panel on that card only. Gate green. Burning-intent buyers during the gap: off-platform payment + scripts/grant-access.ts (the flow that served the first 4 payers). NOTE for Adam: the page's pre-existing "Early access — free while in beta" banner still discloses free beta above the cards; remove it if the intent-gating should be total.

## CONTEXT SWITCH — 2026-07-02 (early: first-run guide + seen-aware drill ordering; UNCOMMITTED on top of the `fixes/2026-07-01-batch` branch / PR #516)

Two beta-feedback issues from Adam ("new users get lost" / "questions repeat too often"), both shipped and browser-verified; gate green (content 0 errors, tsc, **373 tests**, eslint, build):
1. **First-run guide** (`src/app/(app)/dashboard/FirstRunGuide.tsx`): a "First steps" card on the dashboard mapping the first hour — skim a chapter, run a short practice set, set the baseline when ready, come back for review — each with live done-states + direct CTAs. Skippable forever ("Skip the guide" writes top-level `user_metadata.guide_dismissed_at` — deliberately NOT nested under `onboarding`, which auth.updateUser would clobber wholesale). Renders in BOTH dashboard layouts: the empty-state branch (above the baseline hero, softening the "go take a 2h15m exam" first ask) and the has-data branch (until 3 sessions or a baseline exists). Verified live with a disposable user: renders with all 4 CTAs, skip hides immediately AND survives reload, baseline hero unaffected.
2. **Seen-aware drill ordering** (`pickFreshOrder` in `src/lib/topic-skill.ts` + wiring in `practice/session/[slug]/page.tsx`): topic drills serve the whole bank file, and ordering had no memory — file order below the adaptive threshold (a new user redoing a set started at q1 again; this was the repeat complaint), same-bucket reshuffles above it. Now: never-attempted questions first (in adaptive order), then seen ones least-recently-attempted first; RC/MSR groups stay atomic (seen if ANY member attempted, stamped with the newest member attempt). The existing 2k-attempt query was extended (question_id + created_at, desc) — no new query. Deliberately unchanged: chapter tests (fixed curated sets), mocks (already rotate via mockIndex), review queue (repetition is the point). 7 unit tests in `tests/topic-skill-order.test.ts`.
- Files: dashboard/page.tsx (guide wiring in both branches), dashboard/FirstRunGuide.tsx (new), practice/session/[slug]/page.tsx, lib/topic-skill.ts, tests/topic-skill-order.test.ts (new). Commit onto the PR #516 branch or separately — Adam's call.

4. **DI charts now render inside chapter readers** (Adam: "some DI questions refer to a graph but there is no graph"). Content was clean (audit: every graph-referencing question in the bank has a chartSpec or table) — the bug was the surface: `toReaderQuestion` in chapters/[slug]/page.tsx stripped `chartSpec`, and ChapterReader had no chart rendering, so all **46 GI-question embeddings** across di-foundations / graphics-interpretation / di-timing-mixed showed "the graph shows..." with no graph. Fixed: chartSpec threaded through ReaderQuestion + rendered via the shared QuestionChart in both reader card spots (inline checks/pretest + problem-set modal). Verified: the GI chapter's page payload now serializes 56 chartSpec objects into the reader (was zero); visual render uses the same component proven in practice/mock/error-log. (Preview-tab caveat: the reader body defers behind a visibility gate in the headless preview — pre-existing, also affects chartless chapters — so verification was at the props/payload layer.)
   **DISCOVERED SIBLING BUG, chipped for follow-up**: 56 Two-Part Analysis embeddings (two-part-analysis + di-timing-mixed chapters) render in the reader as single-select cards with the answer table stripped and `correctAnswer: -1` — NOTHING ever grades correct in the TPA chapter's reader. Needs the two-column UI ported from SessionClient (chip task_fd94919c has the full brief). **FIXED — see item 5.**

5. **TPA questions now render + grade inside chapter readers** (the chip from item 4, executed later on 2026-07-02; UNCOMMITTED — Adam reviews). All 56 Two-Part Analysis embeddings across two-part-analysis + di-timing-mixed now show the two-column selection grid in BOTH reader card spots (inline pretest/check card + problem-set modal runner) and grade correct when both columns match `twoPartCorrectAnswers` — same semantics as SessionClient's proven TwoPartGrid, restyled as `ReaderTwoPartGrid` on the reader's `--read-*` theme vars. Wiring: `twoPartColumns`/`twoPartCorrectAnswers` threaded through `toReaderQuestion` → `ReaderQuestion` (the chartSpec pattern); shared `isReaderAnswerCorrect` helper; `QuestionProgress` gains optional `twoPartSelections` (`selected` stays null for TPA); `chapter-progress-merge.ts` counts TPA selections in `questionScore` so cross-device merge keeps them (regression test added to `tests/chapter-progress-merge.test.ts`). Existing saved progress is untouched — old entries have no new field, and server-side consumers (`calibration.ts`, `mastery.ts`) already skip `selected: null` entries, which also means submitted TPA answers stay INVISIBLE to calibration + the concept-mastery gate; grading them there via `twoPartSelections` is a clean follow-up if wanted. Verified: parser audit over both chapters — 56/56 TPA embeddings well-formed (2 columns, in-range answer indexes, `correctAnswer: -1`, pipe table stripped from prompt), 10 single-select embeddings exercise the untouched path; gate green (content 0 errors, tsc, **376 tests**, eslint, `next build`). Browser drive stopped at /login: no local test credentials, and disposable-prod-user creation was denied by the session's permission policy — to eyeball it, log in on the running dev server and open `/chapters/two-part-analysis` (pretest card) or its problem sets (modal).

3. **Reader questions no longer reappear in chapter tests** (Adam's follow-up: "end of chapter there was one question, then in the test the first question was the same"). Root cause in `getPracticeChapterGroups` (content.ts): problem-set pins put reader questions INTO the chapter's test pool, tests are difficulty-sorted slices, and reader questions skew easy — so Test 1 literally opened with the pretest/problem-set question the student just answered. Fix: all reader-embedded ids (pretest + recall checks + problem sets, across all chapters) are excluded from test pools, with a MIN_TEST_POOL=4 fallback so tiny chapters keep a test. **516 questions moved out of tests** (tests: 166 → 125; the questions stay in readers + topic drills; bank unchanged at 1,933). Verified on the reported case: quant-05's Test 1 now has ZERO overlap with its 9 reader questions and opens at arithmetic-q34. Regression test over the real bank: `tests/chapter-test-composition.test.ts`. Note: existing users' test compositions shift (beta-acceptable); 2 new validator warnings flag thin chapters (quant-06-fractions-decimals 11q, verbal-20-rc-answer-traps 6q) — grow those banks when convenient. Gate green: **375 tests**.

## CONTEXT SWITCH — 2026-07-01 (FINAL: new-signup health check → 2 product bugs FIXED + adversarial review round; addendum to the two entries below)

Read this with the two blocks below it. Three parallel sessions converged on ONE working tree today (this main session + the two follow-up chips Adam clicked); everything reconciled, no conflicts, gate green at **366 tests**. This block records what the entry below (changeset c) doesn't.

### The signups that triggered all this (all healthy; real people)
4 unknown accounts in 26h: Jose Pablo (Jul 1), Seyram Ametepeh (Jul 1, FIRST-ever marketing opt-in, arrived via a ChatGPT referral to /gmat-verbal-practice), mariam (Jul 1, Quant mock, target 805), Marko Modos (Jun 30, healthiest — 4 sessions, retakes, 2 chapters read). The 2 Jun-29 Google-Ads leads have NOT converted to accounts. **Finding for Adam: signup is effectively OPEN in prod** — `NEXT_PUBLIC_SIGNUP_GATED` only inlines at BUILD time, so the invite gate is dead code in the live client bundle (the entry below calls the gated path "the LIVE one"; it is not — these signups came through the ungated client path). Decide whether open signup is intended; if not, set the var at build time and redeploy.

### Bug 1 (not covered below) — abandoned sessions recorded blanks as wrong answers; FIXED in 3 layers
Ending practice/review early persisted untouched questions as attempts (null answer, 0ms, is_correct=false) with inconsistent totals — poisoned per-topic accuracy + the skill model (live case: mariam's review stored 0/10 @0% off 1 answered question; an April session stored 1/12 @100%).
- Client: `SessionClient.saveSession` persists ONLY submitted questions via pure, unit-tested `src/lib/practice-save.ts::summarizeAnsweredAttempts`; zero answered = no POST. (Runtime browser drive was blocked by a preview-tab hydration quirk; semantics proven by unit tests + the extraction keeps the JSX wiring trivial.)
- Server: `/api/practice-sessions` drops stale-pre-deploy-client blanks via `dropBlankAttempts` (contributed by the chip session; verified wired + mock-exempt).
- Mock timeout scoring (blanks = wrong) is deliberately unchanged everywhere.
- History: **`scripts/cleanup-blank-attempts.ts`** (dry-run default) — 56 blank attempts across 6 sessions: 3 recomputes, 3 all-blank deletions (incl. 2 legacy diagnostic-di). **Adam runs `--apply`.** Documented limitation: selected-but-never-submitted historical rows are indistinguishable from real answers and are left alone.

### Adversarial review round (4 lenses, 3-skeptic verification) — 3 real defects in the fresh fixes, all FIXED after the entry below was written
1. **Cron drip-burst** (supersedes the cron description below): re-anchoring only the enrol window meant a late email-confirmer would get ALL 5 signup-drip emails in one batch (startIso stayed on created_at → every step past-due → Phase 2 sends them together). Window AND `startIso` now share the `max(created_at, email_confirmed_at)` anchor.
2. Same burst class in `/api/marketing-consent` (callable anytime): drip anchors at signup only within 24h of account creation, else starts now.
3. **Cleanup script could have destroyed real data**: an errored remaining-attempts read looked identical to "all blank" and `--apply` would have deleted the session, real answers included. Every read/write is now checked; error_tags on blank attempts are deleted first; failures skip the session and exit 1. Dry-run re-validated against prod after the rewrite.

### Closed out late on 2026-07-01 (Adam said "solve it all")
- **Cleanup APPLIED to prod**: 56 blank attempts across 6 sessions — 3 sessions recomputed (mariam's review now honestly 0/1 @0%; the two 1/1 @100% repairs), 3 all-blank artifact sessions deleted (2 legacy diagnostic-di + 1 custom). 0 failures; re-run confirms 0 blanks remain. If a stale pre-deploy client writes new blanks before changeset (c) deploys, just re-run `--apply` — idempotent.
- **Open signup: DECIDED — stays open.** Ads + the 7-day-trial pivot exist to convert strangers; the invite gate was a private-beta artifact. The gate code stays in the tree (dead in the built bundle) in case it's ever re-armed: doing so needs `NEXT_PUBLIC_SIGNUP_GATED=true` at BUILD time + `SIGNUP_ACCESS_CODE` + disabling public signups in the Supabase dashboard.
- **Committed + pushed as branch `fixes/2026-07-01-batch`** (off `docs/handoff-2026-07-01`): three path-scoped commits — (a) growth copy, (b) paywall hardening, (c) session-integrity + signup-consent — plus a docs commit for this file. One PR, reviewable commit-by-commit; Adam merges (merge = prod deploy).
- The scheduled `verify-seyram-drip-enrolment` check (Jul 2, 16:15 CEST) still stands — the deployed cron does NOT yet have the burst fix; harmless for Seyram (created & confirmed same minute; anchor identical either way), one more reason to merge promptly.
- Both chip-spawned sessions are still running; their fixes are already superseded by this branch. When they finish, discard duplicates; cherry-pick only anything genuinely new.

## CONTEXT SWITCH — 2026-07-01 (LATEST: signup-consent → email_subscriptions gap CLOSED [a THIRD uncommitted changeset]; seyram back-linked; cron-enrolment check scheduled)

The working tree now holds **THREE** separable uncommitted changesets (no file overlap): (a) growth-copy fixes and (b) paywall hardening — both described in the next block — plus (c) **signup marketing-consent**, which an earlier session left implemented but undocumented; this session verified it, fixed the live data, and ran the gate.

### The gap (c) closes
A signup with the marketing box ticked stored consent only in `user_metadata.marketing_consent` — nothing wrote `public.email_subscriptions` at signup time, and a pre-existing subscription row for that email kept `user_id` NULL until the daily 14:00 UTC cron (deployed main) re-derived both from metadata (cron Phase 1a already did `recordConsent` → backfill `user_id` → `enqueueDrip`; the gap was the up-to-24h window, plus the enrol window aging out late-confirming accounts).

### Changeset (c) files (UNCOMMITTED; Adam reviews + commits)
1. **`src/app/api/marketing-consent/route.ts` (NEW)** — POST; records the caller's OWN consent into the ledger via `recordConsent` + enrols the signup drip immediately. Consent facts come from the server-verified auth user's metadata, never the request body; unticked = hard no-op; `recordConsent` still never resurrects an unsubscribe.
2. **`src/app/(auth)/signup/page.tsx`** — after the ungated signup returns a session, a ticked box fires `POST /api/marketing-consent` (fire-and-forget; cron remains the safety net, e.g. the email-confirmation path where no session exists yet).
3. **`src/app/api/signup/route.ts`** (gated path — the LIVE one, `NEXT_PUBLIC_SIGNUP_GATED=true`) — records consent + enqueues the drip inline with the service client right after `createUser`.
4. **`src/app/api/cron/outreach/route.ts`** — enrol window now anchored on `max(created_at, email_confirmed_at)` so accounts confirming days later don't age out unenrolled.

### Live verification + data fix (this session, 2026-07-01 ~21:30 UTC)
- `seyram.e.am@gmail.com` signed up 15:45 UTC via the gated path (auto-confirmed), metadata `optedIn: true` — AFTER that day's 14:00 cron, so nothing had enrolled her yet: subscription row `6019e4b8-…` was `subscribed=true`, `consent_source lead-capture:newsletter` (she opted into the newsletter one minute before signing up), `user_id` NULL; `email_queue` empty. Exactly the reported gap.
- **Back-linked her row** to user `6093c526-…` (guarded one-off update, only fills NULL `user_id`, consent fields untouched). She was the only unlinked row in the table.
- Ledger keeps the earlier `lead-capture:newsletter` consent source by design (`recordConsent` preserves the original consent record); drip emails will cite that consent reason.
- The DEPLOYED cron will enqueue her 5-step signup drip at 14:00 UTC 2026-07-02 and send the due `welcome` step in the same run. A one-time scheduled task **`verify-seyram-drip-enrolment`** (fires 16:15 CEST 2026-07-02, read-only) checks `email_queue`/`email_events` and reports; if rows are missing it debugs the Phase 1a gates.
- Gate green over the combined tree: content 0 errors, tsc, **366 tests**, eslint, `next build`.

## CONTEXT SWITCH — 2026-07-01 (LATER: PR #514 IS NOW MERGED + this session closed the #509 follow-ups and the 4 broken-promise landing pages. Changes UNCOMMITTED in the working tree)

Read this block, then the big pivot entry just below it. Two amendments to that entry:
- **PR #514 is MERGED** (`main` @ `95a71b1`, deployed; everything still dormant behind `PAYWALL_ENABLED` off). Step 1 of its go-live order is done. Tests are now **354**.
- **The "Offered, not done" list shrank**: the 4 newsletter pages' broken-promise copy is fixed by THIS session (below). Still genuinely not done: `--revoke` on grant-access.ts, an `/admin` lookup UI, enforcing the N-month access cutoff.

### This session (UNCOMMITTED; Adam reviews + commits/merges)
Working-tree changes (also reachable on branch `growth/handoff-followups`; a concurrent handoff-docs session later moved HEAD to `docs/handoff-2026-07-01` — the changes ride the working tree either way):
1. **4 broken-promise landing pages fixed** (`/gmat-study-plan-2-months`, `/gmat-data-sufficiency-practice`, `/gmat-verbal-practice`, `/gmat-focus-edition-changes`): each promised a non-existent asset (worksheet / cheat sheet / one-pager) on the `newsletter` magnet — no download, no drip. All repointed to `error-log-template` with page-matched lead copy, same pattern Adam merged in PR #510. Alternatives per page (build the promised asset / drive to signup) remain open — swap before merging if preferred.
2. **`FoundingOffer` success copy now truthful** (closes #509 follow-up): was "I'll email your founding code before paid plans open" — nothing sends that email without opt-in. Now: "Your founding rate is reserved to this address. When paid plans open, I'll send your code here — one email, just the code." That send is the manual owner step at flip time (transactional fulfilment of the reservation; no marketing consent needed).
3. **Orphaned README deleted** (`public/downloads/zakarian-gmat-error-log-readme.md`; closes #509 follow-up): linked from nowhere; its six-tag + how-to content already lives on `/error-log-template`; delivering it free would undercut the 4-part-walkthrough opt-in incentive — the exact 0%-opt-in metric being fixed. The CSV keeps its CTA line.
- Gate green: content 0 errors, tsc, **354 tests**, eslint, `next build`. Browser-verified: all 4 pages render the new lead block; the form POSTs `leadMagnet:"error-log-template"`; success screen shows the CSV download + second-chance opt-in; FoundingOffer renders the new success copy (stubbed `/api/lead-capture` in-browser — no prod rows written).

### Two steps MISSING from the go-live order below — add them at flip time
- **Free-beta copy sweep**: ~15 marketing surfaces, outreach templates, the CSV's last line, and FAQ answers still say "free while it's in private beta" (`grep -ril "free beta\|free while\|private beta" src public` to enumerate). Flipping the paywall with that copy live is a broken promise at scale. (`FoundingOffer` itself is rendered gated on `!PAYWALL_ENABLED`, so it disappears automatically.)
- **Send founding codes**: one manual transactional email per `lead_captures` row with `source='founding-member'` — exactly what the new success copy promises.

### PAYWALL HARDENING (same session, later — review fixes on the SAME uncommitted working tree)
An adversarial review + live paywall-on browser testing of the merged trial gate found real gaps; all fixed and re-verified (gate green: content 0 errors, tsc, **356 tests**, eslint, `next build`; every behavior below observed live with `PAYWALL_ENABLED=true` locally, disposable prod-auth test user created/deleted, `.env.local` restored byte-identical):
1. **Tamper-proof trial start** — `user_metadata.trial_started_at` is client-writable (any signed-in user can reset their own clock via `auth.updateUser`; the client signup stamp also trusts the user's clock). New `entitlements.ts::trialStartFor`: when **`PAYWALL_TRIAL_EPOCH`** (ISO instant) is set, trial start = `max(user.created_at, epoch)` — both server-side, metadata ignored, nothing to backfill. Unset → legacy metadata path unchanged. Verified live: a user whose metadata claimed an 8-days-ago trial was correctly trialing 7 days from `created_at` under the epoch. **`scripts/backfill-trial-start.ts` is now the legacy fallback — setting the epoch replaces it.**
2. **API routes gated** — the proxy matcher excludes `/api`, so a blocked user could still read/write study data (verified live pre-fix: a trial-expired user wrote `target_score` with a 200). New `entitlements.ts::blockIfNoAccess` (402 mirror of the page gate) applied to all 12 study-data handlers: chapter-progress, error-tags (POST+DELETE), mixed-review, mock-flags, mock-review-edits, official-exams, onboarding, practice-sessions, saved-for-review, spaced-review, target-score. Account/billing/settings APIs deliberately NOT gated. Verified live: same writes now 402.
3. **Tutor route bug** — `/api/tutor` gated on the RAW purchase tier, so a TRIALING user would have been denied the AI tutor at flip. Now reads `effectiveTierForUser` (trialing = paid), matching the 5 page gates.
4. **Post-payment race fixed** — `success_url` went straight to gated `/dashboard`; if the redirect beat the webhook, a just-paid customer bounced off the paywall. New gate-exempt **`/purchase-success`** return page retrieves the session from Stripe server-side, validates paid (+ `no_payment_required` for 100%-off codes) and user-match, records the purchase synchronously via shared `src/lib/purchases.ts::recordPurchase` (upsert on `stripe_session_id` — webhook and return page converge; webhook refactored onto the same helper), THEN forwards to `/dashboard?purchase=success&plan=` (ConversionTracker params kept). Verified live: bogus/missing/unpaid session ids all end at `/upgrade` with **0 purchase rows written**; a real checkout session now carries `success_url=/purchase-success?session_id={CHECKOUT_SESSION_ID}` and metadata user-match.
5. **`/settings` exempt from the block** (account management for blocked users — GDPR hygiene); exemption list in `proxy.ts::GATE_EXEMPT_ROUTES` = `/upgrade`, `/purchase-success`, `/settings`. Verified live: blocked user loads `/settings` 200, still bounced from study routes.
6. Smaller: `/upgrade` shows "Choose a plan to unlock the full platform" for never-started accounts (no false "trial has ended"); cancelled checkout from `/upgrade` now returns to `/upgrade` (whitelisted `cancelPath` through CheckoutButton→PricingCard→/api/checkout; verified on the live Stripe session's `cancel_url`); `grant-access.ts` session ids timestamped so revoke-then-regrant can't hit the unique index; deleted the unused PR-#513 `accessFromStripeSubscriptionStatus` mapping + its tests (one-time model won; NOTE: the entry below says #513 was "closed" — its merge is in fact in main, the code was just dead).

**Go-live order below is SIMPLIFIED by #1:** instead of running the backfill script at flip, set **`PAYWALL_TRIAL_EPOCH=<flip instant>`** in Vercel alongside `PAYWALL_ENABLED=true`. Existing users trial from the epoch, new users from signup, nobody can self-extend.

**This working tree now holds THREE separable uncommitted changesets**: (a) growth-copy fixes — 4 landing pages + FoundingOffer + deleted README; (b) paywall hardening — entitlements/proxy/upgrade/purchase-success/checkout/webhook/purchases-lib/11 API routes/2 scripts/tests; (c) abandoned-session blank-attempts fix (next block). Only overlap: (b) and (c) both touch `src/app/api/practice-sessions/route.ts` (independent hunks — `blockIfNoAccess` vs the blank guard). Commit separately by path if wanted.

### ABANDONED-SESSION BLANK ATTEMPTS FIX (changeset (c); completes an earlier interrupted session)
Abandoning a practice/review session partway used to persist every untouched question as a `practice_attempts` row (`selected_answer` null, `time_spent_ms` 0, `is_correct` false), and the stored session trio was inconsistent (`total_questions`/`correct_count` covered ALL questions while `accuracy` covered answered ones — prod session `cbd741b7*` shows correct_count 1, total 12, accuracy 100). Artifact sessions like 0/10 @ 0% (prod `74377552-a028-4a98-b63c-1c7240fc9bb7`, review-quant-2026-07-01) poison per-topic accuracy, the review queue (blanks count as misses and reset spacing rungs), spaced-review concept stats, mastery gates, and study-plan weak areas.
- **Client**: `SessionClient.saveSession` persists SUBMITTED questions only, via new pure `src/lib/practice-save.ts::summarizeAnsweredAttempts`; an all-blank session saves nothing; the stored trio always derives from the answered set so the three fields agree.
- **Server**: `/api/practice-sessions` mirrors the semantics via `practice-save.ts::dropBlankAttempts` for non-`mock-*` slugs — drops no-answer-AND-no-time attempts, recomputes the trio from what remains, writes no row when nothing answered (`{sessionId: null}`). This covers stale pre-fix tabs that still POST blanks after deploy. Submitted two-part answers are never dropped (`selected_answer` null but time > 0). **Mock payloads are exempt on purpose**: a timed-out mock scores unanswered as wrong for exam realism, and MockRunner's trio is deliberately accuracy-of-attempted over total-of-section.
- **Readers left unchanged deliberately** (surveyed: review-queue, spaced-review, mastery, calibration, study-plan-engine, adaptive-plan-engine, analytics, error-log, mixed-review): they count `is_correct=false` rows as misses with no blank filter, but a reader-side `selected_answer IS NOT NULL` filter would also drop legitimate two-part attempts and intentional mock unanswered rows. Fix is at the write path + historical cleanup instead. (`topic-skill.ts` was already safe — it skips attempts under 1s.)
- **RUN AT DEPLOY: `scripts/cleanup-blank-attempts.ts`** (dry-run default; `--apply` to write) — deletes historical blank rows, recomputes each affected session's trio, deletes sessions with nothing answered left, skips `mock-*`. Until it runs, the historical rows keep distorting the readers above.
- Gate green: content 0 errors, tsc, **366 tests** (10 new in `tests/practice-save.test.ts`), eslint, `next build`. Not browser-e2e'd (a real save needs a signed-in Supabase session; the pure helpers carry the semantics and are unit-tested).
...
**Launch readiness:** `scripts/launch-check.ts` is a runnable go-live verifier — `node --experimental-strip-types --no-warnings --env-file=<env> scripts/launch-check.ts`. It checks the go-live env + assets (Supabase core, Stripe secret + 4 prices non-placeholder, webhook secret, site URL, Resend email, cron secret, Sentry DSN, `public/score-report.png`), context-aware on `PAYWALL_ENABLED` (Stripe = blocker when on, warning when off), and exits non-zero on any blocker. **Run it against the PROD env before promoting / flipping the paywall** (local `.env.local` is missing prod-only vars like site URL / Resend / cron / Sentry). Remaining Adam-only go-live items: add a redacted `public/score-report.png`; provision/forward `hello@zakariangmat.com`; set the Stripe price IDs + webhook secret in Vercel before paywall-on; the Vercel cookie alert above.
...
**CONCURRENCY WARNING.** Another agent/process was active on THIS SAME checkout during the session — it switched branches and merged PRs mid-stream (that is how PR #479 auto-merged at an intermediate commit, stranding two later commits onto a new branch). If branches change under you, that is why. Run agents one at a time, or in separate `git worktree`s.

**Already merged to `main`:**
- **Trust/consistency pass (PR #477 / #478).** `1,150+` questions -> a single shared **`1,900+`** claim (`src/lib/site.ts`; live bank ~1,933). Refund copy unified to the 14-day / no-score-guarantee `/refund` policy (killed the "30-day exam refund" FAQ + "Score guarantee on Intensive"). "Free while in beta" framing + early-access banner on `/pricing` (note `PAYWALL_ENABLED` is OFF — the product is free right now). `/students` rewritten honest (founder 565->735 is the only real lift; fabricated student counts removed). Contact/footer/legal email -> **`hello@zakariangmat.com`** (via `SITE_CONTACT_EMAIL`). Sample-chapter "Critical Reasoning" mislabels -> "Verbal Foundations". Homepage "+95 average improvement" -> founder-specific proof. Stale counts fixed (test-builder comment, the "49 chapters" paywall string, AGENTS.md 17->62 chapters). Diagnostic-copy cleanup: the in-app `/diagnostic` route is gone (baseline = official mba.com exam entered on `/mock`); removed stale "diagnostic" copy across how-we-compare / onboarding / personas / study-schedule / layout / manifest / 3 blog posts / adaptive-plan headline; the `free-diagnostic-share.ts` + `/api/og/diagnostic-result` island is orphaned-but-kept (documented in those files).
- **#12 (spaced-review recency) is FIXED + merged (PR #479).** The bug-hunt's deferred #12 is done — `buildSpacedReviewQueue` now orders `error_tags` by the parent practice session's `created_at` (no schema change needed). **The bug-hunt entry below still says "#12 deferred" — it is no longer deferred.**
- **Percentile tables corrected + score-calculator "Likely range" line removed (PR #479).** `src/lib/score-percentiles.ts` rebuilt from the 2020-2025 GMAT Focus population tables (715 was wrongly 92 -> now 99). + `tests/score-percentiles.test.ts`.
- **Standing decision — KEEP "100th percentile" for the founder's 735.** Adam confirmed his official score report shows it; do NOT "correct" it to 99th/99.5th. Backed by a `ScoreReportProof` component on `/about` that renders only once `public/score-report.png` exists (graceful — no broken-image glyph until the file is added). Also recorded in memory `feedback_gmat_percentile_100th`.

**OPEN PRs (clean / mergeable; left open on purpose — merge deliberately, given the auto-merge surprise above):**
- **PR #481** — score calculator shows **100th percentile for 735+** (lifted the `Math.min(99, ...)` display cap; `percentileBand` gains a 100th band) + **`/practice` list now mirrors `/chapters` order 1:1** (the 11 no-bank chapters moved from `OMITTED_CHAPTERS` -> `COMING_SOON_CHAPTERS` in `src/lib/practice-tests-map.ts`, so they render as "coming soon" rows in CHAPTER_PATH_ORDER position; only `gmat-welcome` stays off practice). Tests updated.
- **PR #482** — mobile fixes: homepage horizontal scroll (the `ScoreCalloutNumbers` Results callout was a ~435px non-wrapping inline-flex -> shrank the mobile sizing behind `sm:`; page overflow 76px -> 0) + exam-day "Print" button (`href="javascript:window.print()"`, blocked on iOS/CSP -> new `PrintButton` client component with `onClick`).
- **PR #483** — recall-check answers now hide behind a **"Reveal answer"** toggle (`src/lib/recall-reveal.ts::transformRecallChecks` + `RevealBlock`/`detectReveal` in `ChapterReader`; conservative — only splits checks that cleanly end in `)`, the irregular ones are left inline; +6 unit tests) + the `gmat-welcome` intro chapter now tells the student to **answer every question** on the baseline official exam (an unanswered question hurts more than a wrong one on the Focus Edition).

**Deploy blockers (Adam, before promoting):** (1) add a redacted `public/score-report.png` (activates the 100th-percentile proof on `/about`); (2) provision / forward `hello@zakariangmat.com` (or contact/legal emails bounce); (3) verify the Stripe price env vars — checkout 503s on placeholder price IDs.

**Open follow-ups (not done):** sweep the ~handful of irregular recall checks for full reveal coverage; eyeball the reveal toggle + the new practice "coming soon" rows on the deploy (auth-gated `/chapters/[slug]` + `/practice`); lower-severity mobile polish from the audit (a few sub-44px tap targets, 12-13px body copy, fixed-width rows on auth-gated app screens — not breaking; viewport meta + hamburger nav + scrollable tables are all fine). Gate was green at each step (`npm run check` -> 302 tests by session end, eslint clean, `next build` clean).
...
- **Marketing copy accuracy**: "17/Seventeen chapters" → **"50+ chapters"** site-wide (real rendered count is **58**, not the 59 the prior pricing page claimed; Adam chose approximate phrasing to avoid stale exact counts — see memory `feedback_gmat_marketing_counts`). Legit content ("seventeenth century" RC passages, "seventeen subskills" guide) left alone.
- **Pricing rework #2 (supersedes the guarantee model)**: Self-Study **$429 = 4-month** access; **$599 = "Self-Study + Mentorship"** (6-month + **WhatsApp Q&A** = the only added benefit, stays the "Recommended" highlighted tier); **score guarantee REMOVED from every tier** incl Coaching/Intensive. Cascaded to: /pricing (tiers + comparison table + FAQ + trust strip), homepage 4-tier preview (was stale at **$297**/old-3-tiers), marketing /faq, /terms ("No guarantee of results"), a **full /refund rewrite** (14-day money-back on every plan + package terms; no score guarantee — Adam must review, legal/money), and the dashboard+settings plan-label maps. Coaching/Intensive platform access set to **6mo/12mo (ASSUMPTION — Adam to confirm)**. Internal Stripe id `self_study_guaranteed` + STRIPE_PRICES.selfStudyGuaranteed kept so checkout mapping is intact — only display copy changed.
- **Chapters → single guided path**: new **`CHAPTER_PATH_ORDER`** constant in content.ts (58 slugs: foundations → strategy methods → rotating Quant/Verbal/DI, advanced+timing last); `getAllChapters()` now sorts by it (drives chapters page, practice grouping, etc.). /chapters page rebuilt from 3 per-section journeys into **ONE ordered journey** with a section tag per card; removed now-dead SectionHero/SectionMilestone (~270 lines). **GOTCHA**: chapter `slug` ≠ filename for DI (`di-1-foundations.md` → slug `di-foundations`, `di-2-data-sufficiency.md` → `data-sufficiency`, etc.) — CHAPTER_PATH_ORDER must use the real parsed slugs (caught via node check; Quant/Verbal slugs DO match filenames).
- **Study-plan lighter declutter**: removed the **Exam Readiness card** + the **Mastery-progress** (per-topic Concept/Timed/Mixed gates) panel; fixed the dynamic section-numbering registration so there's no phantom number gap; added a real mba.com link. Kept Today's-focus / weekly calendar / weak-areas / persona / adaptive-plan spine. (Now-unused `MasteryCard` component + dead `masteries` compute left in place — flagged for a prune.)
- **Dashboard**: removed the "Readiness → Target" score-projection cell, added a **"Course progress" %** = chapters fully read (all sections) ÷ 58 (keeps the inline TargetScoreControl); added a real **"Get the exam on mba.com"** external link on the baseline card.
- **Course page**: curriculum collapses to ~2 topics/module with **Show more / Show less** (new client component `src/app/(marketing)/course/CurriculumTopics.tsx`).
- **BUG — chapter→practice 404 (systemic, pre-existing, NOT from the reorder)**: `ChapterCompletionCard` + `ChapterRightPanel` hardcoded `practiceSlug = section` → linked to `/practice/session/quant` (and `/verbal`, `/di`) → `getQuestionsBySetSlug("quant")` = [] → `notFound()` on EVERY chapter. Fixed: compute `firstPracticeTestSlug = getPracticeChapterGroups().find(g => g.chapterSlug === slug)?.tests[0]?.id ?? null` in the server page `chapters/[slug]/page.tsx`, thread **page → ChapterReader → ChapterRightPanel** (the panel renders INSIDE the reader, not from the page), link to `ch-<slug>-t1` with a **`/practice` fallback** (9 method/foundations/timing chapters have no bank). Adversarially verified (0 chapters still 404).
- **`^2` math rendering**: authors write exponents as ASCII caret (`x^2`) in prose AND inline-code chips; pipeline is ReactMarkdown + remarkGfm only (no math). New rehype plugin **`src/lib/rehype-caret-sup.ts`** rewrites `base^exp` → `base<sup>exp</sup>` (handles `^2`, multi-digit `^83`, parenthesized `^(1/2)`/`^(x+1)`, negative `^-3`, single-letter `^n`; **skips** fenced `<pre>` code, `^{` LaTeX, `^*emphasis*`). Wired into **all 26 ReactMarkdown call-sites across 10 files** via an additive `rehypePlugins={[rehypeCaretSup]}` prop (codemod). **Zero content edits** — all ~2490 carets across 55 files fix themselves. Unit-tested every case + end-to-end verified (renders `<sup>`, incl. inside chips) via a temp public route (deleted).

### Verified
`tsc` clean; `validate:content` 0 errors; public pages (pricing/homepage/course/faq/refund/terms/glossary) all 200 with **no** stale guarantee/lifetime/17-chapter copy; chapter path order verified via node (58 slugs, interleaved); 404 fix verified via node (quant-16 → resolvable `ch-quant-16-functions-sequences-t1`, test-less chapters → `/practice`); math fix unit-tested + end-to-end. Auth-gated pages (dashboard/chapters/study-plan/reader) verified via tsc + JSX review + node checks (can't render standalone in preview).

### Follow-up commits on the branch (after the initial push)
- **`7ab46bf`**: added the real "Get the exam on mba.com" link on the **/learn** baseline card (the dashboard + study-plan ones were already done); removed the now-dead `MasteryCard` component from study-plan.
- **`3629e5a`** — **TESTS (addresses the roadmap "ZERO tests exist" item)**: set up **Vitest** (`vitest.config.ts` — node env, `@/`→src alias, `tests/` excluded from the app tsconfig so `tsc` is unaffected; `npm test` = `vitest run`). **13 test files, 181 tests, all green**, mutation-checked to catch real regressions. Coverage: scoring (single-source, both re-exports), chapter guided-path ordering, **chapter→practice slug resolution (regression guard for the bare-section 404)**, **rehype-caret-sup (the math fix)**, entitlements, review-queue, gamification, official-exam scheduler, practice-tests-map/topic-chapter-map, mock helpers. Wrote the 4 core lock-in files by hand; fanned out the other 8 modules via a Workflow (each agent self-verified its file green). Note: gamification's `current`/`activeToday` streak tests are computed relative to the real "today" (the module reads the wall clock internally) — deterministic but a tiny midnight-boundary caveat. Untested (need Supabase/heavy mocking, intentionally skipped): `getPlanTierForUser`, `getReviewQueue`, `computeStudyPlan`'s DB-shaped path.

### Post-merge follow-up branches (each its own PR off main; feedback-batch itself was MERGED as #432)
- **`claude/dashboard-query-parallel`** — perf: the dashboard's ~13 independent per-user Supabase reads now run in ONE `Promise.all` batch (processing logic byte-identical; verified tsc + `next build` + gate; can't preview auth-gated, residual risk noted in the PR description).
- **`claude/chapters-path-order-fix`** — **the guided path was STILL section-grouped on the live page**: `/chapters` re-sorted items by section + title after `getAllChapters()`, destroying the interleave (Adam caught it). Fixed (interactive chapters keep loader order; only guide lists sort), plus the dashboard "next chapter" suggestion now follows the path. **Verified on the REAL rendered page** via a temp public QA route that wraps the actual ChaptersPage component (deleted after) — lesson: verify the rendered surface, not just the loader.
- **`claude/tutor-rate-limit`** — the roadmap's #2 pick: per-user AI-tutor rate limit (default 20/hour, 100/day; env-overridable via `TUTOR_HOURLY_LIMIT`/`TUTOR_DAILY_LIMIT`) + "ai-tutor" added as a PaidFeature (403 once paywall is on; no-op today). New `tutor_usage` table migration (`20260612000000_tutor_usage.sql`, RLS insert/select-own, append-only) + `lib/tutor-rate-limit.ts` (pure evaluator unit-tested; 9 tests, suite now 190). **FAIL-OPEN until Adam applies the migration in Supabase** — applying it is what arms the limit. Usage is recorded BEFORE the model call so abuse is capped even when upstream calls fail. (MERGED by Adam mid-session.)
- **`claude/quant-decimal-free-explanations`** — the roadmap's "broader fraction/decimal audit", DONE via two workflows: a 12-bank audit (31 raw findings → 11 refuted by adversarial judges → **16 confirmed questions**) then per-file fixers + per-question re-verification (2 verifier catches repaired, incl. a real math error). Rewrites use cross-multiplication / cancel-before-multiply / clear-decimals-x100 / benchmark + perfect-square bounding. **Answer keys, prompts, options, credited letters byte-identical to main** (checked mechanically); validate 0 errors. Quant-only ON PURPOSE — DI has an on-screen calculator, Quant doesn't.
- **`claude/practice-link-routing`** — a **link-integrity audit** (6 scanners over every internal href vs a ground-truth route+slug inventory; every finding adversarially verified; 9 confirmed, 0 refuted) found the chapter-CTA 404's bug class in SEVEN more surfaces: the root cause is **chapter slugs / raw session slugs used as `/practice/session/` targets** (the stale "chapter slugs = topic file slugs" assumption; for 11 of 16 topics every Drill CTA 404'd). Fix: new **`TOPIC_TO_SET`** map in `topic-chapter-map.ts` (topic label → question-bank set slug) consumed by mistake-insights (drills + PriorityFix.drillSlug → also heals next-best-action), the in-session weakest-topic CTA, study-plan WeakArea (new `setSlug` field; `chapterSlug` stays for Read links), adaptive-plan-engine (signal carries BOTH `topicSlug` chapter + `setSlug`; question-set activities + /practice recommendations use setSlug — this also fixed a latent bug where /practice recommendations silently dropped all Quant/Verbal topics), spaced-review (question items derive the bank from the question-id prefix, concept items via TOPIC_TO_SET), error-log retake button (non-resolvable mock-/review-/diagnostic session slugs now fall back to the question's own bank or hide), review/question back-link (routes slug families to their real surfaces: mock-* → /mock/report, review-{section}-* → /review/{section}, etc.). Footer's dead `href="#"` X/LinkedIn icons removed (re-add when real profile URLs exist). +3 regression tests (every TOPIC_TO_SET target resolves; id-prefix === setSlug invariant; key parity) — suite 193. NOTE: `/qa/psychometrics` was checked — it's a legit admin tool, proxy-gated; its in-page check fails open (`if (user)`) — minor hardening deferred.

- **`claude/quant-decimal-free-explanations` follow-up commit `d16f29c`** (pushed AFTER Adam merged #436 — needs its own tiny merge): two more verifier catches rescued from stray late agent writes — arithmetic q68 said the fractions "sit near 0.29" (that's their gaps; values sit near 0.71) and backsolving's mistake_a claimed a swapped-price setup "lands inside the range" (it gives 2,976, past the ceiling). Both recomputed by hand.
- **`claude/stale-surface-fixes`** — low-severity audit leftovers + the lessons→chapters repoint: dashboard QuickActions ("Continue Studying" /lessons→/chapters; "Take Mock Exam" /test-builder→/mock); dead `#score-goal` anchor now exists (id on the dashboard course-progress cell; 3 referrers); glossary "Diagnostic" entry rewritten for the official-exam pivot (link → the official-practice-exams blog post, was the deleted /free-diagnostic); `/qa/psychometrics` admin check now FAIL-CLOSED (`!user || !isAdmin` → notFound). **Study-plan now runs on chapters, not the deprecated lessons library**: "Upcoming lessons" panel → "Upcoming chapters" (first 3 incomplete guided-path chapters from `chapter_progress.sectionsRead`), StatCard → "Chapters read x/58", `buildWeeklyCadence(plan, nextReadings)` reading queue emits `/chapters/` days (type "chapter"; "lesson" removed from DailySuggestionType), lesson_completions query dropped from the page. Engine tests rewritten for the new contract (16 tests; suite 194). NOTE: the /lessons pages themselves still exist (only links were repointed); /learn still references lessons in its outline — remaining surfaces are listed in that page if Adam wants a full retirement.

- **`claude/lessons-retirement`** — the two delegated product items, DONE: **(1) /lessons retired** — routes + `/api/lesson-completions` deleted; 308 redirects added (`/lessons→/chapters`; section lessons → their entry chapters: 03→quant-05, 04→verbal-01, 05→di-foundations; 02/06→/mock; catch-all→/chapters; curl-verified); robots + proxy lists cleaned. Lesson .md content files + `getAllLessons` loader KEPT (harmless; historical `lesson_completions` reads on the dashboard still work — note: the lesson-count badges are now legacy/unearnable for new users, prune whenever). **(2) "8 modules" narrative replaced** with the guided-path story on homepage + /course: "One guided path. Five phases." (Foundations first / Strategy toolkit / Core topics in rotation / Advanced + traps / Timing) — /course's phase cards derive example-chapter titles from `getAllChapters()` at render so they can't go stale; old module 04 even advertised "SC", which doesn't exist on Focus. Verified live in preview (no "8 modules"/"Eight modules"/"8 progressive" anywhere; real chapter titles render; collapsible works); gate 194 green; `next build` clean (stale `.next/types` tsc errors right after route deletion are expected — rebuild regenerates them).
- **`claude/gi-chart-renderer-2026-05-24` VERDICT (only remaining branch)**: 6 commits of real value — a `QuestionChart` renderer + `chart-spec.ts` + 49/50 GI questions migrated to chart specs (main renders GI as text/tables today). BUT it's 3 weeks stale and main's `graphics-interpretation.md` has since diverged by +895/−403 lines (enrichment merges); both sides rewrote the same file, and the chart specs were authored against the OLD questions — semantic chart-vs-question mismatches are likely even if the merge applies textually. DO NOT auto-merge: reviving needs a deliberate rebase + visual QA of the 50 charts. Adam's call: revive (worth it for DI quality) or delete the branch.

- **`claude/section-intro-chapters`** (Adam's ask: a general GMAT intro first, then a section primer before each section's first chapter): **4 new authored chapters** — `gmat-welcome` ("Welcome to the GMAT", section: General — full orientation: format/scoring/what-the-exam-rewards/platform loop/realistic prep arc/first-week actions), `quant-section-intro`, `verbal-section-intro`, `di-section-intro` (format + pacing + what the section rewards + how that stretch of the path is organized; explicitly complement, don't duplicate, the existing Foundations chapters). Authored via a write→adversarial-verify→repair Workflow with facts cross-checked against repo sources (mock.ts constants, scoring.ts, focus-vs-old blog); one repair (verbal intro cited the LEGACY 6-tag error taxonomy — `error-log/constants.ts` marks it superseded by ERROR_TAG_DEFS); one manual fix (DI "never groups" vs MSR's consecutive set). Wiring: CHAPTER_PATH_ORDER inserts (path now 62: welcome, quant intro, arithmetic..., verbal intro before verbal-01, di intro before di-foundations), all four in OMITTED_CHAPTERS (no banks), **ParsedChapter.section widened to `Section | "General"`** (consumers handled: allocator skips General, reader/right-panel CTA says "Start practicing", dashboard resume + next-chapter handle it, mixed-review derives section). Tests updated (62, welcome first, intros asserted immediately-before their sections; smoke no longer hardcodes a count). Verified: gate 195 green, build clean, REAL rendered /chapters order + REAL reader render of gmat-welcome via temp QA routes (deleted).

### Open / Adam's call
- Merge `claude/feedback-batch`: `https://github.com/adamik771/gmat-platform/compare/main...claude/feedback-batch?expand=1`
- **Review**: the /refund rewrite (legal/money) and the Coaching/Intensive **6mo/12mo** access assumption.
- Deferred/flagged (not done): homepage/course **"8 modules"** thematic narrative (left — internally consistent with "50+ chapters", different granularity); study-plan **"Upcoming lessons"** panel + the **/learn** page still point to the legacy `/lessons` library; dead `MasteryCard` in study-plan; optional `<sup>` CSS polish inside monospace code chips.

---

## CONTEXT SWITCH — 2026-06-12 (UX de-overwhelm + monetization wiring + pricing rework; ALL MERGED to main)

Long session. Everything below is **merged to main** (PRs #391-#431) — no open branches. `gh` is not installed; PRs are opened from the compare URL and merged in the browser by Adam.

### Shipped this session (all live on main)
- **Practice UX + exam mode** (PR #391): TEST_CAPS 18/10/10 → 9/8/8 (155 short tests); practice rows show no time, show a per-row `aim X/Y` chip from `target_score`; **SessionClient defaults to exam mode** (feedback deferred to end-of-session; study toggle persists via localStorage `session-feedback-mode`); wider chapter reader.
- **Official-exam baseline pivot** (PR #391): the in-app **diagnostic was deleted** (routes + `/free-diagnostic`; 308 redirects `/diagnostic→/mock`, `/free-diagnostic→/signup`). Measurement is now the **6 official mba.com practice exams** entered on `/mock` (the "Official Exam Plan", `OfficialExamPlanClient`, stored in `user_metadata.official_exam_scores`). `lib/diagnostic.ts` kept (mock report + scoring import it). Nav: "Diagnostic" gone, Mock renamed **"Exams"**. Marketing swept for the pivot. Global security headers added (HSTS/nosniff/X-Frame-Options/Referrer/Permissions). Security/payments audit done: Stripe flow is solid (sig verify, idempotency, plan whitelist, RLS) — no critical findings.
- **PR backlog triage**: reviewed ~290 stale auto-generated PRs; merged the valuable ones (#287/#312/#354/#355/#373/#379/#387/#393/#398-402/#277 etc. — CR/quant enrichment, hints, chapter rebuilds), salvaged fixes into `claude/enrichment-fixes` (#404) + the TPA Q6 answer-key / verbal-02 refill / MSR fixes. **Most of the ~280 others should be CLOSED** (duplicate clusters, geometry re-adds, stale bases) — see that chat for the verdicted list. The PRs are generated by **claude.ai/code scheduled routines that are STILL RUNNING** — Adam must turn them off or the backlog refills (new ones appeared mid-session).
- **Four local scheduled tasks created** (run on Adam's machine via the app, not cloud): `gmat-site-health-check` (daily 7am), `gmat-seo-report` (Mon 8am), `gmat-marketing-post` (Mon 9am, one reviewed blog PR/week), `gmat-qa-audit` (Wed 9am). Manage in the app's Scheduled sidebar; Adam should "Run now" once each to pre-approve tools.
- **Study-hours chart** on the dashboard (PR #412): per-day bars, week-over-week comparison, back-nav.
- **De-overwhelm pass** (PR `claude/dedupe-analytics-errorlog`): analytics 15→7 panels; error-log dropped the duplicated "By Section" tab; **dashboard rebuilt minimal** (~17→~7 sections: compact greeting, status strip merging Score-Goal+This-Week, study-hours, slim "View analytics" link, one "Up next" card folding the 4 lower cards, achievements collapsed to a chip); **getting-started collapsed** to a slim "Finish setup" strip (only incomplete steps); **3-step setup** (baseline exam removed as a setup step) + **dashboard official-exam reminder** ("Up next" row when the next weekly official exam is within a week / overdue).
- **Email reminders** (PR #427, INERT until configured): `lib/email.ts` (Resend REST, no-ops without `RESEND_API_KEY`), `lib/reminder-emails.ts`, `/api/cron/reminders` (daily Vercel cron in `vercel.json` at 13:00 UTC; 503s without `CRON_SECRET`; reuses `lib/official-exams.ts` `officialExamReminder()`; idempotent per slot/stage via `user_metadata.reminder_state`; respects new `exams` notification pref). `OfficialExamPlanClient` refactored to share `lib/official-exams.ts`.
- **Chapter reference rail** (PR for `claude/chapter-reference-rail`): a "The big idea" card pins the chapter's authored `**Mental model.**` callout (fallback: summary) at the top of the right rail; rail widened 230→290, outer cap 88rem→92rem. (Glossary auto-match was prototyped then dropped — only 39 terms, too sparse.)
- **Fraction-explanation fix** (PR for `claude/fraction-explanations-fix`): rewrote arithmetic q42 + q68 ("which fraction is greatest") to use **cross-multiplication** instead of memorized decimals; added fastest_path. (Adam flagged the original leaned on decimals like 1/14 nobody knows by heart — likely a recurring pattern worth a broader sweep.)
- **Pricing rework** (PRs #430/#431): merged the two confusing self-serve tiers into one, then added a premium one. **Final 4 tiers: Self-Study $429 (lifetime, only non-guaranteed tier) · Self-Study Guaranteed $599 ("Recommended", + score guarantee + priority async support, no calls) · Coaching $2,500 (+8 sessions) · Intensive $4,200 (+16 sessions).** Guarantee cascades to all $599+ tiers. Feature lists fixed to be true (59 chapters, 1,150+ questions; dropped false "8 modules"/"500+"/mock-count/"community"). Guarantee terms (verifiable from platform data: weak-area chapters to 100%, 800+ questions, 4+ official exams, fixed target, 6-month window) are a **DRAFT Adam must review** (his money). Stripe now needs **4** prices: `STRIPE_PRICE_SELF_STUDY`, `_SELF_STUDY_GUARANTEED`, `_COACHING`, `_INTENSIVE`.

### Monetization is built but OFF — flip order matters
The **paywall scaffold + gates are merged** (`lib/entitlements.ts` + `UpgradeGate`): `PAYWALL_ENABLED` env (default OFF = everything free, current behavior). When on, gates analytics, mock simulator, review queue, test-builder, and practice-tests-beyond-the-first; the free/paid line lives in `entitlements.ts`. Checkout/webhook exist but Stripe keys are placeholders, so **nothing can be charged yet**.

### STANDING ITEMS — only Adam can do these (the real blockers)
1. **Kill the claude.ai/code scheduled routines** (they keep generating duplicate PRs). 2 min, do first.
2. **Email go-live**: Resend account + verify `zakariangmat.com` domain (DNS), then set `RESEND_API_KEY`, `EMAIL_FROM` (`Zakarian GMAT <noreply@zakariangmat.com>` — space, not underscore), `CRON_SECRET` in Vercel Production, redeploy.
3. **Stripe go-live**: 4 products/prices in Stripe → 4 `STRIPE_PRICE_*` env vars + `STRIPE_SECRET_KEY` + `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` + webhook (`/api/stripe/webhook`, `checkout.session.completed`) → `STRIPE_WEBHOOK_SECRET`. For the **30% early-access discount**, create a Stripe **promo code** (don't put price/discount on the site — Adam's call) and share 1:1; the checkout route needs ~3 lines to pass a promo code through (not yet added).
4. **Flip the paywall**: set `PAYWALL_ENABLED=true` AFTER Stripe works (else free users hit a wall with no way to pay). Tune the free/paid line in `entitlements.ts` first if desired.
5. **Review the guarantee terms** on `/refund` (financial commitment).

### REMAINING UPGRADE ROADMAP (Adam ranked email #1, done)
- **AI tutor**: built, gracefully 503s without `ANTHROPIC_API_KEY`. Next-highest: add a per-user rate limit, gate as paid, enable. (Was the #2 pick.)
- **Tests + error monitoring**: ZERO tests exist; add Vitest over pure logic (scoring, `official-exams` scheduler, entitlements, review-queue) + Sentry.
- **Performance**: dashboard/analytics fire many sequential Supabase queries — parallelize.
- **Marketing copy-accuracy sweep** (flagged, not done): FAQ/homepage/course/blog still say **"17 chapters" / "Seventeen chapters"** and old mock claims — the product has **59 chapters / ~1,150 questions**. Pricing page is now correct, creating an inconsistency. Quick high-value pass.
- **Outreach**: Adam is messaging warm leads (people who asked him for GMAT advice) — drafted 3 message templates (close friend / acquaintance / broadcast) with a vague early-access discount + "message me free anytime" offer; in that chat.
- **Broader fraction/decimal explanation audit**: q42/q68 were likely not the only explanations leaning on un-memorizable mental arithmetic.

### Gotchas (still true)
- Auth-gated app pages (dashboard/analytics/chapters/session) can't be rendered standalone in preview — verify via `next build` + reading the JSX + temp QA routes for isolated components (delete after). Screenshot tool frequently renders **black** — use DOM/`preview_eval` reads instead. Temp QA route folders must NOT be `_`-prefixed (App Router treats `_`-folders as private → 404).
- `npm run validate:content` must stay 0 errors. Node `--experimental-strip-types` QA scripts in `/tmp` must import `src/lib/content.ts` by ABSOLUTE path. `scripts/cr-batch-proposals.json` stays UNTRACKED (provenance) — don't `git add -A` it in.
- The parsed question object exposes the credited letter as `correctAnswerLetter`; `**topic:**` → `question.subtopic` (routing key).
...
1. **Product completeness (prior waves 10–13)** — the final PDF-v2 / research-report deltas. Shipped via 4 parallel agents: MAE trend line chart on `/analytics` (wave 11), remediation-assigned/completed flags on `/error-log` (wave 10), role-based admin upgrade via trusted `app_metadata.role` (corrected from the insecure historical `user_metadata` design), and a consolidated SQL migration batch file at `supabase/migrations/20260421000000_wave_batch_8_17_23_28_34_43.sql` (wave 9) covering the 6 pending ALTERs from waves 11, 20, 26, 31, 37, 46.
...
12. **Role-based admin upgrade** — centralized `isAdmin(user)` helper in `src/lib/admin-auth.ts` checks trusted `app_metadata.role === 'admin'` first, then falls back to `ADMIN_EMAILS`. Never use editable `user_metadata` for authorization.
...
2. **Set `ADMIN_EMAILS` env var in Vercel** — comma-separated list (e.g. `ADMIN_EMAILS=adamzakaryan17@gmail.com`). It is the bootstrap fallback for `isAdmin`; the primary role belongs in server-controlled `app_metadata`, set only through an admin/service-role operation. Never put authorization roles in Raw user metadata.
...
- **RLS + full role system** — admin checks use trusted `app_metadata.role === 'admin'` with `ADMIN_EMAILS` fallback. Graduate to database-backed staff roles and matching RLS policies when staff joins.
...
Closes the "Role-based admin system" follow-up flagged since wave 8. Security correction: `ADMIN_EMAILS` remains a fallback and the primary role is trusted `app_metadata.role === 'admin'`; editable `user_metadata` must never grant privileges.

Changes:
- **`src/lib/admin-auth.ts`** — exports `isAdmin(user: User | null): boolean`. Returns true only for server-controlled `app_metadata.role === 'admin'` or an email in `ADMIN_EMAILS`; self-editable `user_metadata.role` is explicitly rejected and test-covered.
...
Promoting a user to admin: use `ADMIN_EMAILS`, or a trusted service-role call such as `supabase.auth.admin.updateUserById(id, { app_metadata: { role: 'admin' } })`. Do not use Raw user metadata.
...
Not done this wave:
- **Full admin role system** — email matching works for a solo-author platform; staff expansion should use trusted `app_metadata` or a dedicated roles table plus RLS, never editable `user_metadata`.
