...
- **Score-goal-setting UI** — once the user has an estimate, the Score Goal card's CTA flips to "Set target" but the target side still shows `—`. Needs a modal or settings row to persist a target (likely in `user_metadata.target_score` via Supabase auth update) and a proper progress bar comparing estimated to target.
