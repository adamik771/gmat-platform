...
And yes — it *must* be below $60,000, by the fulcrum logic: the CEO sits far above the mean, so removing that high weight lets the fulcrum of the remaining group settle lower. Any time you delete an above-average element, the average of what's left drops; delete a below-average element and it rises. This directional rule lets you answer the "is it higher or lower?" half of many DS questions without computing the exact number. Watch the edge case the GMAT loves to test in Data Sufficiency: removing an element exactly *equal* to the mean leaves the average unchanged. So "the new average is lower" is only *guaranteed* when you know the removed element was strictly above the old mean — if a statement leaves that in doubt, the data may be insufficient even though it feels decided.
...
The takeaway you must overlearn: a single far-out value moves mean and range hard, but leaves median and mode flat. The GMAT writes a whole class of DS questions around exactly this asymmetry.
...
**The "mean = median" relationship.** Mean equals median in any symmetric set — evenly spaced, or mirror-paired around a center. On Data Sufficiency, the statement "the set is an arithmetic sequence" (consecutive integers, consecutive multiples, any constant step) is sufficient to conclude mean = median, because arithmetic sequences are always symmetric about their center. The reverse is *not* guaranteed: mean = median does **not** force the set to be evenly spaced. {2, 2, 5, 8, 8} has mean = median = 5 without being arithmetic; {1, 2, 3, 4, 100} is right-skewed (mean 22, median 3), so coincidence is not automatic. Treat "evenly spaced ⟹ mean = median" as strictly one-directional. The skew direction is itself testable: if the mean exceeds the median, the set is **right-skewed** (a tail of large values dragging the mean up); if the mean is below the median, it's **left-skewed**.

**Worked example (DS-style, medium).** Is the mean of set S greater than its median?
- (1) S consists of consecutive even integers.
- (2) S has an even number of elements.

Statement (1): consecutive even integers are arithmetic, hence symmetric, so mean = median. The answer to "is mean > median?" is a definite **No** — sufficient. Statement (2): tells you nothing about shape; a right-skewed even-count set has mean > median, a symmetric one has them equal — insufficient. Answer: **A**. Note the lesson: "sufficient" means you can answer the question *consistently*, even when the answer is "no." A flat, certain "No" is just as sufficient as a flat, certain "Yes" — DS rewards certainty, not a particular direction.

**Median under element swaps.** If you replace the largest element with an even larger one, the median doesn't change. Replace an element *below* the median with something even smaller: median still doesn't change. Only changes that move a value **across the middle position** can shift the median. This is the engine behind a huge family of DS questions, so internalize it: *the median is hostage only to the middle, not the tails.*

**Worked example (hard DS — the subtle one that separates 700+ scorers).** Set T has 7 elements with median m. One element is changed. What is the new median?
- (1) The element that was changed was the maximum, and it was increased.
- (2) After the change, T still has 7 elements, all distinct.

Statement (1): increasing the max keeps it the max — the 4th-position element is untouched, so the new median is still **m**. Sufficient. Statement (2): tells you the count and distinctness but nothing about *which* element changed or by how much — the changed element could have been the median itself. Insufficient. Answer: **A**. The trap choice is C, for test-takers who feel they "need both" — but (1) alone fully pins the median because of the across-the-middle principle.
...
**How GMAT tests range on Data Sufficiency.** Range is determined when you know both the exact maximum and the exact minimum — and not before.

**Worked example.** "What is the range of set S?" Statement (1): the smallest element is 4. Statement (2): the largest element is 22. Each alone is insufficient (you need both endpoints). Together: range = 22 − 4 = 18. Answer: **C**. The lesson generalizes: knowing the mean, the count, or the median gives you nothing about range. You need the extremes directly.

> **Recall check.** Statement (1) says set S has mean 50 and 9 elements; statement (2) says the median is 50. Is the range determinable? (No — neither mean, count, nor median touches the extremes; the answer is "insufficient even together." Mean and median pin the center, never the spread.)

**Outlier sensitivity.** Replace 25 with 250 in {3, 7, 11, 18, 25}: range jumps from 22 to 247. The median stays 11. The SD changes too, but the headline point for the GMAT is the contrast between range (violently outlier-sensitive) and median (immovable). GMAT exploits this on medium DS questions where one statement introduces a large outlier — it shifts the range but not the median, making the two statements carry *different* information about spread, which is precisely how a "both needed / each insufficient" structure gets built.
...
**Worked example (hard — backsolving / plug-in tactic on a DS-style comparison).** A question asks: "Set A = {a, a, a, a} and Set B = {b − 6, b − 2, b + 2, b + 6}. Which has the larger standard deviation?" No numbers for a or b are given.
...
**The zero-SD fact, isolated because it is tested directly.** A set has SD = 0 if and only if **every element is identical**. {10, 10, 10, 10} has SD 0 — there is no spread, every value equals the mean. This is the GMAT's favorite "trick" value because students expect SD to be some messy decimal and forget it can be exactly 0. If a data-sufficiency statement forces all elements equal (or you can deduce it), the SD is pinned at 0 — often making a statement sufficient when it looks like it shouldn't be. Conversely, SD = 0 is *only* achievable by identical elements; any two distinct values guarantee a positive SD.
