...
**Trap to watch.** Percent and percentage points are different. If a tax rate rises from 10% to 15%, that's 5 **percentage points** — but a 50% **percent increase** (5/10 = 50%). On GMAT DS in particular, distinguishing these is the entire problem. A sentence like "unemployment rose 2%" is genuinely ambiguous in English: it could mean 2 percentage points (e.g., 5% to 7%) or a 2% relative increase (5% to 5.1%). The test exploits this. When you see a percent *of a percent*, stop and decide which reading the problem intends before you compute — guessing wrong here costs the whole question.

**Trap to watch (the missing base).** "Is 40% a lot?" is unanswerable without knowing 40% *of what*. A percent is meaningless until it's attached to a base. On DS especially, a statement that gives you a percent but never anchors it to a dollar figure or a count is usually insufficient on its own, because the percent could describe any size of thing. Always ask "percent of what?" before treating a percent as usable information. A close cousin of this trap: two percents taken of *different* bases cannot be added or compared directly — 20% of store A's sales plus 30% of store B's sales is not "50% of anything" unless the two stores have the same base.
...
**Dollar-anchor DS pattern.** Many hard Data Sufficiency questions hinge on whether you have a percent *plus a dollar anchor.* A percent alone is insufficient — it could scale to any absolute size. A dollar amount alone is usually insufficient too — no rate context. But the two **together** are typically sufficient. Train yourself to scan each DS statement and ask: "Does this give me a rate, an absolute anchor, or both?" Then check whether the combination pins the answer. The reflex to build: rate + anchor usually points at (C), but always confirm each statement alone really is insufficient first, because the test sometimes slips a complete answer into one statement to catch the autopilot "C."

**Worked example (DS, dollar-anchor logic).** A store raised the price of an item. By what dollar amount did the price increase? (1) The price increased by 15%. (2) The new price is $46.

Statement (1): 15% of an unknown original — could be a 15-cent increase or a 15-dollar increase depending on the base. Insufficient. Statement (2): the new price is $46, but with no rate we can't recover the increase. Insufficient. Together: new = 1.15 × old = 46, so old = 46 / 1.15 = 40, and the increase = 46 − 40 = $6. The rate (1) plus the dollar anchor (2) pins it exactly. Answer: **C** — both statements together, neither alone. This is the dollar-anchor pattern in its purest form: percent + dollar = sufficient, and you can often see it's (C) before doing any arithmetic. (DS reminder: you don't have to finish the division to know it's sufficient — recognizing that one linear equation in one unknown is fully determined is enough.)

> **Recall check.** In Data Sufficiency, why is "a price rose 15%" alone never enough to find the *dollar* amount of the increase? (A percent is a rate with no scale — 15% of $20 is $3, 15% of $200 is $30. Without an absolute anchor the dollar figure can be anything, so a lone percent is insufficient.)
