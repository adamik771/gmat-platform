...
summary: |
  ...
  This is one of the most learnable sections on the exam: the content changes every time, but the process never does. Build the process once and it transfers to every set you will ever see. By the end of this chapter you will: (1) run the two-pass protocol — orient in 30-45 seconds, then read selectively per question; (2) recognize all four MSR question types from the stem alone; (3) execute the three-step synthesis workflow without losing data points across tab switches; (4) parse quantifiers precisely on Yes/No statement checks; and (5) name the five wrong-answer patterns on sight, so you eliminate deliberately instead of second-guessing.
sections:
  - id: pretest
    type: pretest
...
**Mental model.** Reading every tab thoroughly before question 1 is a trap. You'll spend 3+ minutes on content you'll never use. MSR rewards selective reading driven by the question, not comprehensive reading driven by anxiety. Think of the tabs as a reference manual, not a chapter to read cover to cover — you open it to the page you need, when you need it.
...
**Pro tip.** Numbers live in tables; intent lives in memos. Memos describe data in prose but imprecisely. When a question asks for a specific dollar figure or percentage, go to the table. When it asks for rationale or intent, go to the memo. Each source is authoritative for what it's specifically designed to convey — and knowing which source to trust for which kind of answer is half the speed gain in this section.
...
**Micro-drill.** A set has three tabs: Tab 1 = a regional sales VP's memo, Tab 2 = a quarterly revenue table by region, Tab 3 = the board's strategic targets. For each stem, name the single tab you open first — 30 seconds total:

1. "What was the Western region's Q3 revenue?" → ___
2. "Why did the VP attribute the Central decline to seasonality?" → ___
3. "Did the company meet the board's 12% growth target?" → ___
4. "Which region's revenue figure does the VP describe as 'disappointing'?" → ___

Answers: (1) **Tab 2** — a specific number lives in the table. (2) **Tab 1** — rationale and attribution live in the memo. (3) **Tabs 2 and 3** — this is synthesis; you need the actual (table) and the target (board), so flag both. (4) **Tab 1 first** — the *characterization* ("disappointing") is the VP's, so start in the memo to find which region he means, then confirm the figure in Tab 2 only if the question needs the number. If you sent (4) straight to the table, you skipped the step that tells you *which* region the question is even about.
...
**Trap to watch.** Memos characterize; tables quantify. A memo says "Central region revenue declined." A lookup question asking "what was the Central region's revenue decline?" needs the table number, not the memo's characterization. The memo says "declined"; the table says "$4.37M, down from $4.60M, a 5% decline." These are different levels of precision. When the question wants a figure, the prose word is a decoy — go to the table.
...
**Pro tip.** The discipline in step 2 is non-negotiable: write numbers down as you find them. Students who try to hold 3-4 values in memory while switching tabs make errors — not because the math is hard, but because working memory is unreliable under time pressure when you're also navigating tabs. Two seconds of writing buys you a value you can trust; a half-remembered value you have to re-verify costs thirty.
...
**Mental model.** When sources conflict, ask what each source is *for*. A memo says "6% decline"; the table shows 5.8%. The table is authoritative for numbers; the memo is authoritative for reasoning, attribution, and intent. If the question asks for the specific figure, use the table. If it asks why a result occurred, use the memo. The conflict is rarely an error in the set — it's a test of whether you know which source owns which kind of claim.

**Micro-drill.** Use these two tabs. Tab A (table): Eastern revenue this year = $6.96M, last year = $5.80M. Tab B (memo): "$0.90M of Eastern's current revenue came from a single enterprise contract signed in Q1." Answer each — 60 seconds total:

1. Eastern's headline year-over-year growth rate → ___
2. Eastern's organic growth rate, excluding the enterprise contract → ___
3. In one phrase, why do (1) and (2) tell different stories → ___

Answers: (1) **20%** — ($6.96M − $5.80M) / $5.80M. (2) **≈4.5%** — strip the contract from the current year first: ($6.96M − $0.90M − $5.80M) / $5.80M = $0.26M / $5.80M. (3) **one contract, not the underlying business, drove the growth.** This is the exact insight a synthesis question probes — list both numbers, then let the gap between them speak.
...
**Mental model.** Change only what the question specifies, and cascade only to what is logically downstream of it. If the question increases Region X's revenue by $1M, adjust Region X's revenue, adjust any totals that include Region X, and recompute any ratios built from those totals. Do not touch other regions' individual revenues. The skill is drawing the line between "logically affected" and "frozen" — too narrow and you forget the total; too wide and you change things the question never touched.
...
**Trap to watch.** Hypothetical questions often produce answers close to round numbers, and rounding too early lands you on a wrong choice that was placed there for exactly that reason. If you round $8,400 to $8,000 mid-computation, your final answer shifts enough to match a decoy. Keep full precision through every intermediate step; round only at the final comparison, and only if "approximately" appears in the question.
...
**Micro-drill.** Actual value from the table is **$42.00**. For each statement, mark Yes or No — 30 seconds total:

1. "Revenue per unit exceeded $42." → ___
2. "Revenue per unit was at least $42." → ___
3. "Revenue per unit was more than $41.99." → ___
4. "Revenue per unit was exactly $42." → ___

Answers: (1) **No** — "exceeded" is strictly greater than; $42.00 does not clear it. (2) **Yes** — "at least" includes equality. (3) **Yes** — $42.00 > $41.99. (4) **Yes** — exactly $42.00. Same number, four different verdicts. The quantifier, not the data, decides the answer — read it like a contract.

**Trap to watch.** "Every / all / each" requires checking every instance, not a sample. A statement like "every region achieved positive revenue growth" covers all four (or five) regions. Most students check two or three, see yes, and stop. If any single region is flat or negative, the answer is No — regardless of what the others did. Before checking, count how many instances the statement claims to cover, then verify all of them as a checklist.
...
**Mental model.** MSR distractors are not random; they are manufactured from your most likely mistake. Every wrong answer is a real number or a plausible claim that you would arrive at by slipping in one specific way — wrong row, wrong operation, one unchecked case. Learn the five slips and you can read a wrong answer backward to the error it was built to catch.
...
**Pro tip.** Before going to the tab, write down exactly what dimension the question asks for — which region, which year, which metric. Then check that dimension label against the row and column header you're actually reading. The slip happens when you navigate to the right neighborhood and grab the cell next door.
...
**Pro tip.** Confirm what the question is asking for — absolute change, percentage change, or ratio — before computing. Write the formula on scratch paper first, then fill in the numbers. The decoy is almost always your own intermediate result, so naming the final form up front stops you from stopping early.
...
**Pro tip.** Every answer to an MSR question must be anchored in specific tab content. "Sounds plausible" is not a Yes. If you can't point to the exact line in a tab that supports the claim, treat it as unsupported and eliminate it — general knowledge is never the source.
...
**Pro tip.** On any statement with a universal quantifier ("every," "all," "each," "none"), count the instances the statement covers, then verify every one before answering. Treat it as a checklist, not a spot-check — the single case you skip is statistically the one the test put there to fail.
...
**Pro tip.** For any specific numerical answer, go back to the tab and read the number directly rather than recalling it — unless you read it in the last 10 seconds. Memory degrades fast under cognitive load. The tab is always right; your memory of the tab is sometimes not.

## @summary

**Key takeaway.** MSR is a process section, not a knowledge section. The tabs change every time; the workflow never does. Orient, then read selectively per question; match each stem to its question type; write your data points down before you combine them; and read quantifiers like a contract. Internalize those four habits and the content in front of you stops mattering — you'll run any set the same way.
...
- **Easy set first, untimed.** Two questions on a shared Lumen Beverages set. Goal: run pass 1 deliberately, then answer each question by navigating, not re-reading. If you find yourself re-reading a whole tab, stop and ask which single data point you actually needed.
- **Medium set, 90-120 seconds per question.** This is where Simple Lookup and Cross-Tab Synthesis mix. Log which question type each one was and where the time went. Consistently over time usually means pass 1 was rushed — invest the full 30-45 seconds up front and watch the per-question time drop.
- **Hard set, untimed first pass.** Hard MSR is almost always Conditional/Hypothetical or a multi-statement Yes/No check. Write out "Change: ___ / Compute: ___" before touching a tab, and treat every "every/all/each" statement as a checklist. On the second pass, time yourself toward 5-6 minutes for the full set.
- **Below 60% on the medium set:** revisit @cross-tab-synthesis — most medium errors trace back to skipping the write-it-down step and losing a value across a tab switch.
- **Any MSR miss on a practice test or mock:** log the root cause against the five patterns — wrong data point, wrong operation, unsupported claim, unchecked instance, or memory bleed. Each pattern has a specific fix above; cataloguing them tells you which one needs more reps.
- **After this chapter:** the closest sibling is Table Analysis, which uses the same filter-then-verify discipline on a single sortable table. Two-Part Analysis and Graphics Interpretation round out the Data Insights section and are worth scheduling next.
