...
//
// The cross-section weave is proportional (Quant:Verbal:DI ~ 29:20:6), so
// every section runs the full length of the path instead of one clumping at
// the end (previously the 8 RC chapters + late CR walled up as ~13 Verbal
// chapters in a row once Quant/DI were exhausted). Within each section the
// easy->hard sequence is preserved; only the interleaving across sections is
// tuned. When adding chapters, keep all three streams spread to the end --
// don't append a section's new chapters as a trailing block.
